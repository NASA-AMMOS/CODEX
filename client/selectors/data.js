...
export const getFeaturesWithSelected = domain => {
    return getFeatures(domain).map(f => List.of(f, domain.get("selected_features").has(f)));
};
    ...
    */
    ...
    for (let m = 0; m < masterMask.size; m++) {
        ...
        for (let s = sels.size - 1; s >= 0; s--) {
            ...
            if (sel.get("visible") && sel.get("mask").get(m)) {
...
export const getFeaturesMasked = (domain, xfeature, yfeature, type, selectionName) => {
    ...
    if (type === "selections")
        mask = mask.find(obj => {
            return obj.get("name") === selectionName;
        });
