...
export default class Model {
    ...
    hasData() {
        if( this.models[ this.activeModel ] )
            return true;
        return false; 
    }
    ...
    validateData( activeModel = 0 ) {
        ...
        this.models[this.activeModel].data[0] =
            formulas.cleanStringArray( this.models[this.activeModel].data[0] );
        ...
        for( let c = 0; c < this.models[this.activeModel].data[0].length; c++ ) {
            ...
            if( idx !== -1 ) {
                console.warn( 'File Parse CSV - Found a duplicate column: ' + this.models[this.activeModel].data[0][c] );
    ...
    addListener( listenerName, listenerFunction, graphId, windowId, groupId ) {
        if( this.listeners.hasOwnProperty( listenerName ) ) {
            this.listeners[listenerName].push( { call: listenerFunction, graphId: graphId, windowId: windowId, group: groupId || null } );
        }
    }
    ...
    getFileName( activeModel = 0 ) {
        if( this.models[activeModel] )
            return this.models[activeModel].filename;
    }
    ...
    getSubColumns( colNames, subsetId, activeModel = 0 ) {
        ...
        if( subsetId && !Array.isArray( subsetId ) )
            subsetId = [ { selection: subsetId } ];

        let subsetIdIndex = [];
        if( subsetId ) {
            for( let i = 0; i < subsetId.length; i++ ) {
                let ind = parseInt( formulas.objectArrayIndexOfKeyWithValue( this.models[activeModel].subsetIds, 'id', subsetId[i].selection ), 10 );
                if( ind > -1 )
                    subsetIdIndex[i] = ind;
                else
                    i--;
        ...
        for( let i = 0; i < this.models[activeModel].data.length; i++ ) {
            ...
            if( subsetId ) {
                for( let j = 0; j < subsetId.length; j++ ) {
                    ...
                    if( ( this.models[activeModel].subsets[subsetSlot][i] & ( 1 << subsetIdIndex[j] ) ) !== 0 ) {
                        isSet = true;
                        break;
                    }
                }
            ...
            if( isSet ) {
                for( let j = 0; j < colIndexOrder.length; j++ ) {
                    rowData.push( colIndexOrder[j] === -1 ? 0 : this.models[activeModel].data[i][colIndexOrder[j]] );
                }
            }
            else if( !subsetId ) {
                for( let j = 0; j < colIndexOrder.length; j++ ) {
        ...
        if( this.models[activeModel] ) {
            for( let i = 0; i < this.models[activeModel].subsetIds.length; i++ ) {
                if( this.models[activeModel].subsetIds[i] !== null ) {
                    let index = formulas.objectArrayIndexOfKeyWithValue( subsetInfo, 'name', this.models[activeModel].subsetIds[i].id );
    ...
    setFeaturesCustomOrder( newFeaturesCustomOrder, activeModel = 0 ) {
        if( newFeaturesCustomOrder )
            ...
            this.models[activeModel].featuresCustomOrder = this.getColumnInfo();
    }
    setSelectionsCustomOrder( newSelectionsCustomOrder, activeModel = 0 ) {
        if( newSelectionsCustomOrder )
            ...
            this.models[activeModel].selectionsCustomOrder = this.getSubsetInfo();
    }
    setSelectionCustomOrderColor( selection, hex, activeModel = 0 ) {
        let index = formulas.objectArrayIndexOfKeyWithValue( this.models[activeModel].selectionsCustomOrder, 'selection', selection );       
        if( index !== -1 ) {
        ...
        if( features && features[0] ) {
            for( let i = 0; i < features[0].length; i++ ) {
                if( this.models[activeModel] ) {
                    ...
                    this.models[activeModel].featuresCustomOrder.push( { name: testFeatureName, type: '0' } );
                    for( let j = 1; j < this.models[activeModel].data.length && j < features.length; j++ ) {
    ...
    setBrushedRowsArea( mode, range, xAxisFeature, yAxisFeature, group, activeModel = 0 ) {
        ...
        for( let i = 0; i < this.listeners.brushChange.length; i++ ) {
            if( this.listeners.brushChange[i].group != null &&
    ...
    subsetsChanged( changeBy, changedId, changedSlot, activeModel = 0 ) {
        if( changedId === 'obj' ) {
            ...
            if( changedSlot.constructor === Array && changedId.length === changedSlot.length ) {
                ...
                console.error( 'SubsetsChanged Failed' );
        }
        else {
            ...
            invocation.invoke(
                ...
                function() { console.log( 'Subset ' + changedId + ' ' + changeBy + 'ed on the server.' ) }
            );
        }
    ...
    getBinarySubset( slot = 0, activeModel ) {
        ...
        for( let i = 1; i < this.models[activeModel].subsets[sm].length; i++ ) {
            binarySubset.value( i - 1, ( ( this.models[activeModel].subsets[sm][i] & ( 1 << ( slot % 8 ) ) ) !== 0 ) );
        }
        ...
        if( this.models[activeModel] ) {
            ...
            if( subsetSlot !== null ) {
                ...
                if( this.models[activeModel].subsets[sm] === undefined ) {
                    this.models[activeModel].subsets[sm] = new Uint8Array(this.models[activeModel].data.length).fill(0);
                }
                this.models[activeModel].subsetIds[subsetSlot] = { id: subsetId, subId: 0 };
                this.models[activeModel].selectionsCustomOrder.push( { selection: subsetId, 
                    color: controller.vars.selectionsColorPalette[subsetSlot], subId: 0 } );

                for( let i = 1; i < this.models[activeModel].data.length; i++ ) {
                    if( filter( this.models[activeModel].data[i] ) ) {
                        this.models[activeModel].subsets[sm][i] = this.models[activeModel].subsets[sm][i] | (1 << (subsetSlot % 8)); 
                    } 
                    else {
                        this.models[activeModel].subsets[sm][i] = this.models[activeModel].subsets[sm][i] | (0 << (subsetSlot % 8)); 
                    }  
                }
            }
    ...
    */
    addFilter( subsetId, filter, numFilters, numOffset = 0, dontTurnOn, activeModel = 0 ) {
        ...
        if( this.models[activeModel] ) {
            ...
            while( !foundUniqueId ) {
                let wasMatched = false;
                for( let s = 0; s < this.models[activeModel].subsetIds.length; s++ ) {
                    if( this.models[activeModel].subsetIds[s] !== null &&
            ...
            for( let f = 0; f < numFilters; f++ ) {
                ...
                if( subsetSlot !== null ) {
                    ...
                    addedSubsets.push( { selection: subsetIdF, type: 'selection',
                        color: controller.vars.selectionsColorPalette[subsetSlot], parts: undefined } );
                    ...
                    this.models[activeModel].selectionsCustomOrder.push( { selection: subsetIdF,
                        color: controller.vars.selectionsColorPalette[subsetSlot], subId: f } );
                    for( let i = 1; i < this.models[activeModel].data.length; i++ ) {
            ...
            if( !dontTurnOn ) {
                for( let i = 0; i < allUsedSubsetSlotsAndIds.length; i++ ) {
                    controller.addChosenSelection( allUsedSubsetSlotsAndIds[i].id, 'selection',
    ...
    renameSelection( currentName, newName, activeModel = 0  ) {
        ...
        let index = formulas.objectArrayIndexOfKeyWithValue( this.models[activeModel].selectionsCustomOrder, 'selection', currentName );
    ...
    computeFeatureFacts() {
        ...
        for ( let i = 0; i < colInfo.length; i++ ) {
            this.vars.featureFacts.push({
                ...
                mean: 0,
                mode: 0,
                type: (!isNaN(points[1][i]) === true ? (Number.isInteger(points[1][i]) ? "Integer" : "Float") : "Tokenized String"),
                notNumber: 0,
                notNumberPercentage: "",
                inf: 0,
                infPercentage: "",
        ...
        for ( let i = 0; i < points.length; i++ ) {
            ...
            for ( let j = 0; j < featureFacts.length; j++ ) {
                if (isNaN(points[i][j])) {
                    ...
                    let el = points[i][j];
                    if (featureFacts[j].modeMap[el] == null)
                        featureFacts[j].modeMap[el] = 1;
                    else
                        featureFacts[j].modeMap[el]++;  
        ...
        for ( let i = 0; i < featureFacts.length; i++ ) {
            ...
            this.quickselect(featureFacts[i].featureArray, Math.ceil(featureFacts[i].totalDigits/2));
            featureFacts[i].stdDev = this.getStandardDeviation(featureFacts[i].featureArray);
            featureFacts[i].median = featureFacts[i].featureArray[(Math.ceil(featureFacts[i].totalDigits/2))];
            featureFacts[i].notNumberPercentage = String(Math.round(((featureFacts[i].notNumber / totalPoints) * 100) * 100 / 100)) + "%";
            featureFacts[i].infPercentage = String(Math.round(((featureFacts[i].inf / totalPoints) * 100) * 100 / 100)) + "%";
