...
export default class Controller {
    constructor() {
        ...
        this.vars = {
            ...
            selectionsColorPalette: ['#7733e6', '#e63380', '#98e633', '#33e6c5', '#333be6', '#e63333',
    ...
    subscribeChosenFeaturesUpdate( chosenFeaturesUpdateFunction, id ) {
        ...
        this.listeners.chosenFeaturesUpdate.push( { id: id || '', call: chosenFeaturesUpdateFunction } );
    }
    unsubscribeChosenFeaturesUpdate( chosenFeaturesUpdateFunction, id ) {
        ...
        for( let i = this.listeners.chosenFeaturesUpdate.length - 1; i >= 0; i-- ) {
            if( this.listeners.chosenFeaturesUpdate[i] && this.listeners.chosenFeaturesUpdate[i].id === id ) {
    ...
    subscribeChosenFeaturesUpdateMiddleWare( chosenFeaturesUpdateMiddleWareFunction, id, idsToIntercept ) {
        this.listeners.chosenFeaturesUpdateMiddleWare.push( { id: id, middleware: chosenFeaturesUpdateMiddleWareFunction, for: idsToIntercept } );
    }
    unsubscribeChosenFeaturesMiddleWareUpdate( chosenFeaturesUpdateFunction, id ) {
        for( let i = this.listeners.chosenFeaturesUpdateMiddleWare.length - 1; i >= 0; i-- ) {
            if( this.listeners.chosenFeaturesUpdateMiddleWare[i] && this.listeners.chosenFeaturesUpdateMiddleWare[i].id === id ) {
    ...
    callAllChosenFeaturesUpdate() {
        ...
        for( let l in this.listeners.chosenFeaturesUpdate ) {
            ...
            if( typeof possibleMiddleware === 'function' ) {
                possibleMiddleware( () => this.listeners.chosenFeaturesUpdate[l].call(), clearCallbacks );
    ...
    addChosenFeature( feature, type, parts ) {
        if( typeof feature === 'string' && feature.length > 0 &&
            formulas.objectArrayIndexOfKeyWithValue( this.vars.chosenFeatures, 'feature', feature ) === -1 ) {
    ...
    setChosenFeatures( chosenFeatures ) {
        ...
        chosenFeatures = chosenFeatures.filter( function(f) { return f !== undefined } );
    ...
    getChosenFeatures( type ) {
        if( type === 'column' || type === 'subset' ) {
            ...
            return this.vars.chosenFeatures;
        
    }
    removeChosenFeature( feature ) {
        //Get index with feature
        let index = formulas.objectArrayIndexOfKeyWithValue( this.vars.chosenFeatures, 'feature', feature );
    ...
    toggleChosenFeature( feature, forceTo, isShift ) {
        ...
        if( index !== -1 && forceTo !== 'on' ) {
            ...
            this.addChosenFeature( feature, 'column' );
    }
    ...
    subscribeChosenSelectionsUpdate( chosenSelectionsUpdateFunction, id ) {
        this.listeners.chosenSelectionsUpdate.push( { id: id || '', call: chosenSelectionsUpdateFunction } );
    }
    unsubscribeChosenSelectionsUpdate( chosenSelectionsUpdateFunction, id ) {
        for( let i = this.listeners.chosenSelectionsUpdate.length - 1; i >= 0; i-- ) {
            if( this.listeners.chosenSelectionsUpdate[i] && this.listeners.chosenSelectionsUpdate[i].id === id ) {
    ...
    subscribeChosenSelectionsUpdateMiddleWare( chosenSelectionsUpdateMiddleWareFunction, id, idsToIntercept ) {
        this.listeners.chosenSelectionsUpdateMiddleWare.push( { id: id, middleware: chosenSelectionsUpdateMiddleWareFunction, for: idsToIntercept } );
    }
    unsubscribeChosenSelectionsMiddleWareUpdate( chosenSelectionsUpdateFunction, id ) {
        for( let i = this.listeners.chosenSelectionsUpdateMiddleWare.length - 1; i >= 0; i-- ) {
            if( this.listeners.chosenSelectionsUpdateMiddleWare[i] && this.listeners.chosenSelectionsUpdateMiddleWare[i].id === id ) {
    ...
    callAllChosenSelectionsUpdate() {
        ...
        for( let l in this.listeners.chosenSelectionsUpdate ) {
            let possibleMiddleware = this.getSelectionsMiddleware( this.listeners.chosenSelectionsUpdate[l].id );
            if( typeof possibleMiddleware === 'function' ) {
                possibleMiddleware( () => this.listeners.chosenSelectionsUpdate[l].call(), clearCallbacks );
                clearCallbacks = false;
            }
            else
    ...
    addChosenSelection( selection, type, color, parts ) {
        if( typeof selection === 'string' && selection.length > 0 &&
            formulas.objectArrayIndexOfKeyWithValue( this.vars.chosenSelections, 'selection', selection ) === -1 ) {
            this.vars.chosenSelections.push( { selection: selection, type: type, color: color, parts: parts } );
    ...
    setChosenSelections( chosenSelections ) {
        ...
        //remove all undefineds
        chosenSelections = chosenSelections.filter( function(f) { return f !== undefined } );
    ...
    getChosenSelections( type ) {
        if( type === 'column' || type === 'subset' ) {
            ...
            return this.vars.chosenSelections;
        
    }
    removeChosenSelection( selection ) {
        //Get index with selection
        let index = formulas.objectArrayIndexOfKeyWithValue( this.vars.chosenSelections, 'selection', selection );
        if( index !== -1 ) {
    ...
    toggleChosenSelection( selection, color, forceTo, isShift, stopRefresh ) {
        ...
        if( index === -1 && isShift && this.vars.lastChosenSelection !== null ) {
            ...
            for( let i = 0; i < indicesBetween.length; i++ )
                this.toggleChosenSelection( s[indicesBetween[i]].selection, s[indicesBetween[i]].color, 'on', false, true );
        }
        ...
        if( index !== -1 && forceTo !== 'on' ) {
            ...
            this.addChosenSelection( selection, 'selection', color );
    ...
    setSelectionsColorPalette( selection, i, hex ) {
        let selI = formulas.objectArrayIndexOfKeyWithValue( this.vars.chosenSelections, 'selection', selection );
        //Max sure both pass
        if( selI !== -1 && model.setSelectionCustomOrderColor( selection, hex ) )
            this.vars.chosenSelections[selI].color = hex;
        else
            console.warn( 'WARNING - failed to update selection color.' );
    }
    ...
    setDrawers( to ) {
        for( let i in this.genericGraphs ) {
            if( this.genericGraphs[i].setDrawer )
                this.genericGraphs[i].setDrawer( to );
        }
        if( to === 'rectangle' || to === 'freehand' )
            this.vars.lastBrushType = to;
    }
    setTopBarMode( to, brushType ) {
        if( to === 'zoom' ) {
            if( this.vars.topBarMode === 'snap' )
            ...
            else if( this.vars.topBarMode !== 'snap' )
                this.manager.toggleAllWindowSnapPorts( true );
            this.setDrawers( 'clearshape' );
    ...
    setMessage( message, status ) {
        if( this.setMessageText )
            this.setMessageText( message, status );
    }
    toggleIndeterminateLoadingBar( on, message ) {
        if( this.toggleIndeterminateLoading )
            this.toggleIndeterminateLoading( on, message );
    }
    ...
    addGraphReceiveListener( listenerId, listenerFunction ) {
        for( let i = 0; i < this.listeners['graphChange'].length; i++ ) {
            if( this.listeners['graphChange'][i].id != null &&
    ...
    addGraphLinkListener( listenerId, listenerFunction ) {
        for( let i = 0; i < this.listeners['graphLinkChange'].length; i++ ) {
            if( this.listeners['graphLinkChange'][i].id != null &&
    ...
    getAlgoSubalgo( algo, subalgo ) {
        ...
        let i = formulas.objectArrayIndexOfKeyWithValue( algorithm.subalgorithms, 'simplename', subalgo );
        if( i !== -1 )
            return algorithm.subalgorithms[i];
        return [];
    }
    getGraphByType( type ) {
        let i = formulas.objectArrayIndexOfKeyWithValue( this.vars.graphs, 'type', type );
        if( i !== -1 )
            return this.vars.graphs[i];
        return {};
    }
    graphChanged( id ) {
        for( let i = 0; i < this.listeners['graphChange'].length; i++ ) {
            if( ( id === undefined || ( this.listeners['graphChange'][i].id !== null && 
    ...
    getChosenXYSubset() {
        ...
        const selectedFeatures = BANDAID_PATCH_STORE.getState().getIn(['data','selected_features']).toJS();
