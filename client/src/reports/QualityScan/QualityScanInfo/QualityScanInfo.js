...
class QualityScanInfo extends Component {
    ...
    setQualityInfo(type, maskObj, variable) {
        ...
        if (variable === "last") variable = this.vars.lastVariable;
    ...
    getFullMaskFeatureData(maskObj, featureName) {
        ...
        if (rowNumber !== -1) {
            for (let r = 1; r < this.props.data.size; r++) {
                if (maskObj.masker(this.props.data.getIn([r, rowNumber]), rowNumber)) {
                    ...
                    else fullData[this.props.data.getIn([r, rowNumber])] = 1;
                }
            }
        } else {
        ...
        if (cutoff > 0) {
            for (let i in fullData) {
                if (fullData[i] <= cutoff) delete fullData[i];
            }
        }
    ...
    _tablizeData(data, isList) {
        if (this.state.sortBy === 0) data = formulas.sortObjectByKeys(data);
        else data = formulas.sortObjectByValues(data);
            ...
            if (isList)
                tableElements.push(
                    <tr
                        ...
                        onMouseEnter={(function(i) {
                            return () => {
                                that.props.highlightByValue(that.state.list.name, keys[i]);
                            };
                        })(i)}
                        onMouseLeave={() => {
                            that.props.mouseLeftFeature();
                        }}
                    >
                        <td title={keys[i]}>{keys[i]}</td>
                        <td>{data.values[i]}</td>
                        <td title={(data.values[i] / dataLength) * 100 + "%"}>
                            {((data.values[i] / dataLength) * 100).toFixed(2) + "%"}
                        </td>
                    </tr>
                );
            else
                tableElements.push(
                    <tr key={i}>
                        <td title={keys[i]}>{keys[i]}</td>
                        <td>{data.values[i]}</td>
                        <td title={(parseFloat(data.values[i], 10) / valueSum) * 100 + "%"}>
                            {((parseFloat(data.values[i], 10) / valueSum) * 100).toFixed(2) + "%"}
                        </td>
                    </tr>
                );
        }
    ...
    sortTable(by) {
        if (this.state.sortBy === by) this.setState({ sortAscending: !this.state.sortAscending });
        else this.setState({ sortBy: by });
    }
                        ...
                        <div key={i} className="option">
                            ...
                            <div className="range">
                                <input
                                    ...
                                    onChange={e => {
                                        this.vars.lastChangedInput = parseFloat(e.target.value);
                                        this._refs.optionvalues[i].innerText = parseFloat(
                                            e.target.value
                                        ).toFixed(2);
                                    }}
                                    onMouseUp={() => {
                        ...
                        <div key={i} className="option">
                            ...
                            <div className="text">
                                <input
                                    ...
                                    defaultValue={inputs[i].value}
                                    onChange={e => {
                                        this.vars.lastChangedInput = e.target.value;
                                    }}
                                    onBlur={() => {
                                        this.props.setMaskInputValue(
                                            this.vars.maskName,
        ...
        return (
            <div className="QualityScanInfo">
                <div className="infoHeader">
                    <div id="headertitle">
                        {l
                            ? this.vars.maskName + " - " + this.state.list.name
                            : this.state.list.name}
                    </div>
                    <div
                        id="headerclose"
                        onClick={() => {
                            this.props.shiftInfoTable(false);
                        }}
                    >
                        <MdClose />
                    </div>
