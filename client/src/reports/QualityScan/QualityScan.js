...
import { FaCheck, FaSortAlphaAsc, FaSortAlphaDesc, FaSortNumericAsc,
  FaSortNumericDesc, FaCaretLeft, FaAlignLeft, FaRefresh, FaSearchPlus, FaSearchMinus } from 'react-icons/lib/fa';
...
class QualityScan extends Component {
  constructor(props) { 
    ...
    this.masks = [
      { name: 'Infinities', gradient: 'gradients/red.png',  color: 'red', opacity: 1, masker: function( v ) {
      ...
      { name: 'NaNs', gradient: 'gradients/yellow.png',  color: 'yellow', opacity: 1, masker: function( v ) {
        return ( isNaN( v ) ) ? 1 : 0;
      } },
      { name: 'Repeats', gradient: 'gradients/green.png',  color: 'lime', opacity: 0,  masker: function( v, row ) {
        ...
        maskerData: function( rowNumber ) {
          if( this.results.hasOwnProperty(rowNumber) ) { return; }
          let rowRepeats = {};
          let max = -Infinity;
          if( rowNumber !== -1 ) {
      ...
      { name: 'Outliers', gradient: 'gradients/cyan.png', color: 'cyan', opacity: 0, masker: function( v, row ) {
          return ( Math.abs( v - this.results[row].mean ) >
            ( this.results[row].standardDeviation * this.inputs.sigma.value ) ) ? 1 : 0;
        },
        results: {},
        maskerData: function( rowNumber ) {
          if( this.results.hasOwnProperty(rowNumber) ) { return; }
      ...
      { name: 'Custom', gradient: 'gradients/fuchsia.png', color: 'fuchsia', opacity: 0,  masker: function( v ) {
    ...
    this.featureData = { full: [], list: mFeatures, raw: getFeatures(this.props.data).toJS(), map: map };
  ...
  highlightFeature( i ) {
    this.ref_highlighter.style.top = ( this.ref_featureTableTR[i].getBoundingClientRect().top
      - this.ref_QualityScanTable.getTableRef().getBoundingClientRect().top ) -
      this.ref_QualityScanTable.getTableRef().getBoundingClientRect().height  + 'px';
  }
  ...
  makeFeatureTable( featureList ) {
    ...
    for( let i = 0; i < featureList.length; i++ ) {
      featureTableElements.push(
        ...
        >
          <td className='tableFeatureName' title={featureList[i]}>{featureList[i]}</td>
          <td></td>
          <td></td>
        </tr>
      );
  ...
  makeFeatureList( featureList ) { return;
  ...
  _setValuesFor( maskId ) {
    ...
    for( let i in this.masks[maskId].rowHits ) {
      let hidden = false;
      if( (this.featuresHidden && this.masks[this.chosenMask].rowHits[this.featureData.map[i]] === 0 ) ) hidden = true;

      updatedTableFeatureList.push( {
        name: this.featureData.list[i],
        hidden: hidden,
        count: this.masks[maskId].rowHits[ this.featureData.map[i] ],
        percent: (( this.masks[maskId].rowHits[ this.featureData.map[i] ] / this.masks[maskId].dataLength ) * 100 ).toFixed( 2 ) + '%',
        percentRaw: (( this.masks[maskId].rowHits[ this.featureData.map[i] ] / this.masks[maskId].dataLength ) * 100 ) + '%'
      } );
    }
  ...
  finalizeTableHeaderFooter() {
    ...
    if( ftt !== undefined ) { //They're not all invisible
    ...
    totalsbar.children[2].innerHTML = (( this.masks[maskId].totalRowHits / (this.masks[maskId].dataLength * this.masks[maskId].rowHits.length) ) * 100 ).toFixed( 2 ) + '%';
    totalsbar.children[2].title = (( this.masks[maskId].totalRowHits / (this.masks[maskId].dataLength * this.masks[maskId].rowHits.length) ) * 100 ) + '%';
  }


  setExtentBox = ( startRow, pixelsPerBin, dataLength ) => {
    let width = this.ref_chart['horizontal'].getBoundingClientRect().width;
    this.ref_extentbox.style.transition = 'left 0.2s cubic-bezier(0.445, 0.05, 0.55, 0.95), width 0.2s cubic-bezier(0.445, 0.05, 0.55, 0.95)';
    ...
    for( let m = this.masks.length - 1; m >= 0; m-- ) {
      ...
      switch( graphPos.toLowerCase() ) {
        case 'vertical':
          ...
          this.ref_chart['vertical'].parentElement.style.height = this.ref_QualityScanTable.getTableRef().getBoundingClientRect().height + 'px';
          ...
          this.ref_chart['vertical'].insertAdjacentHTML( 'beforeend', '<polyline class="QualityScan_Distribution_' + mask.name + '" fill="#fff" fill-opacity="' + mask.opacity/8 + '" stroke="' + mask.color + '" stroke-width="2" stroke-opacity="' + mask.opacity + '" points="' + ptString + '"/>' );
          break;
        case 'horizontal':
          ...
          this.ref_chart['horizontal'].insertAdjacentHTML( 'beforeend', '<polyline class="QualityScan_Distribution_' + mask.name + '" fill="#fff" fill-opacity="' + mask.opacity/8 + '" stroke="' + mask.color + '" stroke-width="2" stroke-opacity="' + mask.opacity + '" points="' + ptString + '"/>' );
          break;
        default:
  ...
  setMaskOrder( maskOrder ) {
    ...
    if( this.ref_webglheat )
      this.ref_webglheat.setMaskOrder( maskOrder );
  }
  ...
  shiftInfoTable( open ) {
    if( open )
      this.ref_righter.style.left = 'calc( 100% - ' + this.ref_QualityScanTable.getWidth() + 'px )';
    else
      this.ref_righter.style.left = '100%';
  }

  sortFeaturesToggle( headerIndex ) {
    let byFull = false;
    let newOrder;
    if( headerIndex === 0 ) { //sort by name
    ...
    else if( this.featureData.full.length > 0 ) { //sort by count/percent
      byFull = true;
      ...
      console.warn( 'Warning! - tried to sort Quality Scan features by count before counts were computed.' );
      return;
    }
  ...
  extentBoxEvent( eventType, e ) {
    switch( eventType ) {
      case 'down':
          ...
          this.extentStartLeft = parseInt( this.ref_extentbox.style.left.replace('px',''), 10 );
          this.lastExtentBoxX = e.pageX;
        break;
      case 'move':
        if( this.extentBoxDown ) {
          this.ref_extentbox.style.transition = 'none';
          this.ref_extentbox.style.left = parseInt( this.ref_extentbox.style.left.replace('px',''), 10 )
            + (e.pageX - this.lastExtentBoxX) + 'px';
        }
        this.lastExtentBoxX = e.pageX;
        break;
      case 'up':
  ...
  rearrangeFeatureList( newOrder, byFull ) {
    ...
    let newList = [];
    if( byFull ) { //then we need to assemble newList based on newMap
      ...
      <div className='QualityScan'>
        ...
        <div className='right'>
          <div className='top'>
            <div className='optionbar'>
              <div className='optionbarleft'>
                <div ref={(r) => this.ref_option_sortrows = r }
                  onClick={() => this.sortRowsToggle()}
                ><span><FaAlignLeft /></span><div>Sort Rows</div></div>
                <div ref={(r) => this.ref_option_hiddenfeatures = r }
                  onClick={() => this.hiddenFeaturesToggle()}
                ><span><GoFold /></span><div>Hide Empty Features</div></div>
                <div ref={(r) => this.ref_option_hiddenrows = r }
                  onClick={() => this.hiddenRowsToggle()}
                ><span style={{transform: 'rotate(90deg)'}}><GoFold /></span><div>Hide Empty Rows</div></div>
              </div>
            ...
            <div className='header'>
              <table cellPadding='0' cellSpacing='0' border='0'>
                <thead>
                  <tr className='tableHeader' ref={(r) => this.ref_tableheader = r }>
                    <th>
                      Feature
                      <div onClick={() => this.sortFeaturesToggle(0)}><FaSortAlphaAsc /></div>
                    </th>
                    <th>Count</th>
                    <th>
                      Percent
                      <div onClick={() => this.sortFeaturesToggle(1)}><FaSortNumericDesc /></div>
                    </th>
                  </tr>
                </thead>
              </table>
          ...
          <div className='middle'>
            <div className='heat'>
              <WebGLHeat
                ref={(r) => { this.ref_webglheat = (r) ? r.getWrappedInstance() : null; }}
                masks={this.masks}
                rwm={this.props}
            ...
            <div className='vertline'>
              <svg width='100%' height='100%' className='chart' ref={ (r) => { this.ref_chart['vertical'] = r;} }></svg>
            </div>
            <QualityScanTable
              ref={(r) => { this.ref_QualityScanTable = r; }}
              featureList={this.featureData.raw}
            ...
            <canvas id='highlightCanvas'
                ref={(r) => { this.ref_highlightCanvas = r; }}/>
            <div id='highlitArrow'
              ref={(r) => { this.ref_highlitArrow = r; }}></div>
          </div>
          <div className='bottom'>
            <div className='horiline'>
              <div className='extentbox' ref={ (r) => { this.ref_extentbox = r;} }
                onMouseDown={(e) => this.extentBoxEvent('down',e)}
                onMouseMove={(e) => this.extentBoxEvent('move',e)}
                onMouseUp={(e) => this.extentBoxEvent('up',e)}
              ></div>
              <svg width='100%' height='100%' className='chart' ref={ (r) => { this.ref_chart['horizontal'] = r;} }></svg>
            </div>
            <div className='corner'>
              <div id='invertColors' ref={(r) => { this.ref_invertColors = r; }}
                onClick={(r) => { this.invertColors(); }}
              ><MdInvertColorsOn /></div>
            </div>
            <div className='totalsbar' ref={(r) => this.ref_totalsbar = r }><div></div><div></div><div></div></div>
          </div>
        </div>
        <div id='righter' ref={(r) => { this.ref_righter = r; }}>
          <QualityScanInfo
            ref={(r) => { this.ref_QualityScanInfo = (r) ? r.getWrappedInstance() : null; }}
            setMaskInputValue={(m,i,v) => this.setMaskInputValue(m,i,v) }
