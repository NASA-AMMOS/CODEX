...
const SortableItem = SortableElement(({ ix, value, that }) => {
    return (
        <li
            className="QualityScanMasks_maskli"
            style={{ zIndex: 10000, listStyleType: "none" }}
            onWheel={e => that.scroll(e, ix)}
            ref={r => {
                that._refs.masklis[ix] = r;
            }}
        ...
        >
            <div className="header">
                <div
                    ...
                    ref={r => {
                        that._refs.masklispalette[ix] = r;
                    }}
                ...
                <div
                    className="subheader"
                    onClick={() => {
                        that.check(value.name);
                        that.choseMask(ix);
                    }}
                >
                    <div className="Checkbox" ref={r => (that._refs.checks[value.name] = r)}>
                        <div>
                            <FaCheck />
                        </div>
                    </div>
                    <div className="maskNames">{value.name}</div>
                </div>
                <div className="subheader">
                    <div className="opacityslider">
                        <input
                            ...
                            max="100"
                            step="1"
                            defaultValue={value.opacity * 100}
                            onChange={e => {
                                that._refs.masklispalette[ix].style.opacity = e.target.value / 100;
                                that.props.setMaskOpacity(value.name, e.target.value / 100);
                            }}
                            onMouseUp={() => {
                                that.props.setMaskOpacity(value.name, "end");
                            }}
                        />
                    </div>
                    <div className="toggleSettings" onClick={() => that.toggleSettings(value.name)}>
                        <FaCaretDown />
                    </div>
                </div>
            </div>
            <div className="maskmiddle" />
            <div className="lisettings">{that.getSettings(value.name)}</div>
        </li>
    );
});
...
class QualityScanMasks extends Component {
    constructor(props) {
        ...
        for (let i in this.props.masks) {
            masks.push({
                name: this.props.masks[i].name,
                gradient: this.props.masks[i].gradient,
                masker: this.props.masks[i].masker,
                checked: true,
                opacity: this.props.masks[i].opacity
            });
        }
        this.state = {
            masks: masks
    ...
    scroll(e, i) {
        ...
        if (e.deltaY > 0 && h >= 30) this._refs.masklis[i].scrollTop += 30;
        else this._refs.masklis[i].scrollTop -= 30;
    }
    ...
    toggleSettings(name) {
        ...
        if (this._refs.options.hasOwnProperty(name)) {
            ...
            else this._refs.options[name].main.style.display = "flex";
        }
    }
    getSettings(name) {
    ...
    buildOptions(name, inputs) {
        ...
        if (inputs) {
            ...
            if (!this.vars.inputValues.hasOwnProperty(name)) this.vars.inputValues[name] = {};
                ...
                if (inputs[i].type === "range") {
                    inputElements.push(
                        <div
                            key={i}
                            className="option"
                            ref={r => (this._refs.options[name].main = r)}
                        >
                            <div>{i}</div>
                            ...
                            <div className="range">
                                <input
                                    ...
                                    max={inputs[i].max}
                                    step={inputs[i].step}
                                    defaultValue={inputs[i].value}
                                    onChange={e => {
                                        this.vars.inputValues[name][i] = parseFloat(e.target.value);
                                        this._refs.options[name].values[i].innerText = parseFloat(
                                            e.target.value
                                        ).toFixed(2);
                                    }}
                                    onMouseUp={() => {
                                        this.props.setMaskInputValue(
                                            name,
                                            i,
                                            this.vars.inputValues[name][i]
                                        );
                                    }}
                                />
                            </div>
                        ...
                        >
                            ...
                            <div className="text">
                                <input
                                    ...
                                    defaultValue={inputs[i].value}
                                    onChange={e => {
                                        this.vars.inputValues[name][i] = e.target.value;
                                    }}
                                    onBlur={() => {
                                        this.props.setMaskInputValue(
                                            name,
