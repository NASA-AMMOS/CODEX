...
describe("Data store", () => {
    ...
    it("should handle FEATURE_ADD", () => {
        ...
        expect(state.get("data").toJS()).toEqual([["f1", "f2", "f3"], [1, 2, 3], [3, 4, 5]]);
    ...
    it("should handle BRUSH_UPDATE_AREA", () => {
        const act1 = actions.fileLoad(
            [["f1", "f2", "f3"], [1, 1, 3], [2, 2, 1], [3, 3, 2]],
            "test.csv"
        );
        const act2 = actions.brushUpdateArea(
            "rectangle",
            { x: [0.5, 1.5], y: [0.5, 1.5] },
            "f1",
            "f2"
        );
        const act3 = actions.brushUpdateArea(
            "rectangle",
            { x: [0.5, 2.5], y: [0.5, 2.5] },
            "f2",
            "f3"
        ...
        const act4 = actions.brushUpdateArea(
            "freehand",
        ...
        //freehand triangle
        const act5 = actions.brushUpdateArea(
            "freehand",
            [{ x: 1, y: 0.5 }, { x: 2, y: 3 }, { x: 6, y: 1 }],
            "f2",
            "f3"
        );
