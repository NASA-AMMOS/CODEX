...
export default class Formulas {
    ...
    findMaxValueInArray(array) {
        let max = -Infinity;
        for (let i in array) if (max < array[i]) max = array[i];
        return max;
    }
    ...
    stringToEllipsisSubstring(str, place, length) {
        ...
        if (ellipsisSubstring.length >= length) {
            ...
            else ellipsisSubstring = ellipsisSubstring.substr(startI, length);
            /*
            if( startI !== 0 )
    ...
    cleanStringArray(arr) {
        for (let i in arr)
            if (typeof arr[i] === "string") arr[i] = arr[i].replace(/\s/g, "_");
            else arr[i] = "";
        return arr;
    }
    ...
    isPointInPoly(poly, pt) {
        //poly and pt are objects of the form {x: ,y: }
    ...
    sortObjectByValues(o) {
        let s = Object.keys(o).sort(function(a, b) {
            return o[a] - o[b];
        });
        let sorted = {
            keys: s,
