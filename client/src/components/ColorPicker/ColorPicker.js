...
class ColorPicker extends Component {
    ...
    componentDidUpdate() {
        if (!this.updateFlag) {
            if (
                this.props.initialColorI !== null &&
                this.props.initialColorI !== this.state.color
            ) {
    ...
    _makeColors() {
        ...
        for (let i = this.state.colors.length - 1; i >= 0; i--) {
            pickColors.push(
                <li
                    className={"ColorPickerPickColors"}
                    key={i}
                    style={{
                        background: this.state.colors[i],
                        boxShadow:
                            this.state.colors[i] === "none" ? "inset 0px 0px 3px #000" : "none"
                    }}
    ...
    render() {
        ...
        return (
            <div
                ...
                style={{ background: this.state.colors[this.state.color] }}
            >
                ...
                {this.state.displayColorPicker ? (
                    <div style={styles.popover}>
                        <ul className="container">{this._makeColors()}</ul>
                    </div>
                ) : null}
            </div>
        );
