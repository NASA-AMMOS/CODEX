...
// create a
const setup = (data, selectedFeatures, selectionCreate = () => {}) => {
    data =
        data ||
        fromJS({
            data: [["f1", "f2", "f3"], [1, 2, 3], [4, 5, 6], [7, 8, 9]]
        });
    selectedFeatures = selectedFeatures || fromJS(["f1", "f2"]);

    return shallow(
        <Filter data={data} selectedFeatures={selectedFeatures} selectionCreate={selectionCreate} />
    );
};
