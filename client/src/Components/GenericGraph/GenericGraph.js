...
class GenericGraph extends Component {
  constructor(props) {
    ...
    this.graph = getGraphByType(this.props.ui, this.vars.type).get('component') || getGraphs(this.props.ui).getIn([0, 'component']);
  ...
  setData( dataState ) {
    ...
    dataObj.data = getFeaturesMasked( dataState, this.vars.xaxis, this.vars.yaxis, 'master').toJS();
    ...
    switch(this.vars.type) {
      case "scatter":
        ...
        if( this.vars.subsets.length > 0 ) {
          ...
          for( let s = 0; s < this.vars.subsets.length; s++ ) {
            ...
            option.series[index].itemStyle = { normal: {
                opacity: 1,
                color: this.vars.subsets[s].color
              }
            };
            if( this.vars.subsets[s].emphasize )
      ...
      case "bar":
        ...
        option.title.text = this.vars.xaxis.substr(0, 20) + ' vs ' + this.vars.yaxis.substr(0, 20);
        break;
      case "parallel":
  ...
  onBrushDrawn( d ) {
    if( this.vars.type === 'scatter' ) {
      switch( d.mode ) {
        case 'rectangle':
          let convertedRange = [];
          convertedRange[0] = this.echarts_instance.convertFromPixel( { seriesIndex: 0 }, [d.range.x[0], d.range.y[0]] );
          convertedRange[1] = this.echarts_instance.convertFromPixel( { seriesIndex: 0 }, [d.range.x[1], d.range.y[1]] );
          let newRange = { x: [convertedRange[0][0], convertedRange[1][0]].sort(formulas.asc),
        ...
        case 'freehand':
          ...
          for( let p in d.polyPoints ) {
            let cfp = this.echarts_instance.convertFromPixel( { seriesIndex: 0 }, d.polyPoints[p] );
            convertedPoints.push( { x: cfp[0], y: cfp[1] } );
          }
          this.props.brushUpdateArea( d.mode, convertedPoints, this.vars.xaxis, this.vars.yaxis );
          break;
        default:
          break;
      }
  ...
  componentResize( ) {
    if( this.genericGraph ) {
      if( ( this.genericGraph.offsetWidth !== this.lastDimensions.width )
  ...
  _makeInvertAxesMenuItems() {
    ...
    let boo = true;
    if (notInclude.indexOf(this.vars.type) === -1)
      boo = false;
      menuItems.push(
      <MenuItem
        disabled={boo}
  ...
  handleContextMenuClick(e, data) { console.log( 'here' );
    switch( data.action ) {
      case 'changegraph':
  ...
  correctContextMenuPosition() {
    setTimeout( () => {
      if( this.genericGraph && this.genericGraph.parentElement && this.genericGraph.parentElement.parentElement
  ...
  render() {
    return (
      <div className='GenericGraph' ref={(e) => { this.genericGraph = e; }}>
        <ContextMenuTrigger id={'GenericGraphContextMenu_' + this.vars.graphId}
          holdToDisplay={-1}>
          <ReactEcharts ref={(e) => { this.echarts_react = e; }}
              option={this.graph.getOption()}
              style={{height: '100%', width: '100%'}}
          />
          <Drawer ref={(r) => { this.ref_drawer = r; }}
            windowId={this.vars.windowId}
            onDrawingEnd={(d) => { this.onBrushDrawn(d); } }
          />
        </ContextMenuTrigger>
        ...
        <div className='GenericGraphWrapper' ref={(e) => { this.wrapper = e; }}>
          <ContextMenu id={'GenericGraphContextMenu_' + this.vars.graphId}
            className={'GenericGraphContextMenu'}
            onShow={() => this.correctContextMenuPosition()}
            onHide={() => this.hideContextMenu()}>
            <MenuItem data={{ action: 'brushtoselection' }} onClick={this.handleContextMenuClick}>
              Save Selection
            </MenuItem>
            ...
            <SubMenu title='Format Graph' hoverDelay={100}>
              <MenuItem data={{ action: 'format_reset' }} onClick={this.handleContextMenuClick}>
                Reset
              </MenuItem>
              <MenuItem data={{ action: 'format_invertcolors' }} onClick={this.handleContextMenuClick}>
                Invert Colors
              </MenuItem>
            </SubMenu>
