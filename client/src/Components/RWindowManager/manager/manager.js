export default class Manager {
    ...
    addWindow(config, wmId = 0) {
        if (this.wms.hasOwnProperty(wmId)) {
            ...
            console.warn(
                "Manager failed to add a new window. RWindowManager might not have initialized."
            );
    }
    focusWindow(id, groupId = -1, wmId = 0) {
        ...
        if (groupId >= 0) {
            for (let i in this.wms[wmId].windows) {
                if (
                    this.wms[wmId].windows[i]._.embrace.isEmbraced &&
                    this.wms[wmId].windows[i]._.embrace.group === groupId
                )
                    if (this.wms[wmId]._focusWindow[i]) this.wms[wmId]._focusWindow[i](true);
            }
        }

        //Bring window to top
        if (this.wms[wmId]._focusWindow[id]) this.wms[wmId]._focusWindow[id]();

        //Highlight in taskbar\
        if (this.wms[wmId].Taskbar) this.wms[wmId].Taskbar.highlightThumbnail(id);
    }
    ...
    centerWindow(id, wmId = 0) {
        if (this.wms[wmId]._centerWindow[id]) this.wms[wmId]._centerWindow[id]();
    }
    maxmizeWindow(id, wmId = 0) {
        if (this.wms[wmId]._maximizeWindow[id]) this.wms[wmId]._maximizeWindow[id]();
    }
    ...
    tweenWindows(arrangement, wmId = 0) {
        Array.from(document.getElementsByClassName("WindowRnd")).forEach(e => {
    ...
    shiftGroup(groupId, shiftX, shiftY, wmId = 0) {
        if (groupId !== -1) this.wms[wmId].shiftGroup(groupId, shiftX, shiftY);
    }
    removeGroupHandle(groupId, wmId = 0) {
        if (groupId !== -1) this.wms[wmId].removeGroupHandle(groupId);
    }
    removeFromGroup(id, wmId = 0) {
        this.wms[wmId].removeFromGroup(id);
    }
    minimizeGroup(groupId, wmId = 0) {
        if (groupId !== -1) this.wms[wmId].minimizeGroup(groupId);
    }
    removeGroupWithWindows(groupId, wmId = 0) {
        if (groupId !== -1) this.wms[wmId].removeGroupWithWindows(groupId);
    }
    minimizeAll(wmId = 0) {
    ...
    refreshWindow(id, wmId = 0) {
        if (this.wms[wmId]._refreshWindow[id]) this.wms[wmId]._refreshWindow[id]();
    }
    refresh(wmId = 0, timeout) {
        if (this.wms[wmId]._refreshWindows) this.wms[wmId]._refreshWindows();
        if (this.wms[wmId]._refreshTaskbar) this.wms[wmId]._refreshTaskbar(timeout);
    }
    ...
    getFocusOrder(id, isReversed, wmId = 0) {
        //Slow but fine since we won't have thousands of windows
        let orderings = [];
        for (let w in this.wms[wmId].windows) {
