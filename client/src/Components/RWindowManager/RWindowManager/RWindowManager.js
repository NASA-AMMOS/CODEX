...
class RWindowManager extends Component {
    ...
    setWindowState(id, stateName, state, stopRefresh) {
        ...
        this.windows = newWindows;
        if (!stopRefresh) manager.refresh(this.wmId);
    }
        ...
        if (this.ref_path) {
            ...
            if (manager.snap.path.on) {
                elm.setAttribute(
                    "d",
                    new Bezier(
                        manager.snap.path.start.x,
                        manager.snap.path.start.y,
                        (manager.snap.path.start.x + manager.snap.path.end.x) / 2,
                        manager.snap.path.start.y,
                        (manager.snap.path.start.x + manager.snap.path.end.x) / 2,
                        manager.snap.path.end.y,
                        manager.snap.path.end.x,
                        manager.snap.path.end.y
                    ).toSVG()
                );
            } else {
    ...
    invertEmbrace(wmId = 0) {
        for (let w in this.windows) {
            ...
            manager.wms[wmId]._context[this.windows[w]._.id].embrace(
                this.windows[w]._.embrace.isEmbraced
            );
        }
    }
    ...
    getNextFreeGroup() {
        ...
        let nextGroup = 0;
        while (groups.indexOf(nextGroup) !== -1) nextGroup++;
        return nextGroup;
    }
    groupEmbraced() {
        ...
        if (ids.length > 0) {
            ...
            for (let id in ids) {
                ...
                groupXY[0] +=
                    this.windows[ids[id]]._.stateVars.x +
                    this.windows[ids[id]]._.stateVars.width / 2;
                groupXY[1] +=
                    this.windows[ids[id]]._.stateVars.y +
                    this.windows[ids[id]]._.stateVars.height / 2;
            }
        ...
        return (
            ...
            >
                <div id="RWindowManager_windowcont">
                    <Windows
                        ref={r => {
                            this.WindowsComponent = r;
                        }}
                        wmId={this.wmId}
                    />
                    <svg id="RWindowManager_svgunderlay">
                        <path
                            id="RWindowManager_svgunderlay_path"
                            ref={r => {
                                this.ref_path = r;
                            }}
                            d=""
