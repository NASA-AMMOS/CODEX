...
class Taskbar extends Component {
  ...
  setThumbnailState( id ) {
    ...
    if( g !== -1 )
      stateBackground = manager.colorPalette[g];
  ...
  highlightThumbnail( id ) {
    ...
    for( let t in this.ref_thumbnailBar ) {
      if( this.ref_thumbnailBar[t] )
        this.ref_thumbnailBar[t].style.background = 'black';
    }
    //Then highlight
  ...
  makeThumbnails() {
    ...
    for( let w in manager.wms[this.wmId].windows ) {
      ...
      thumbnails.push(
        <div key={id} className='rwm_thumbnail'
          onMouseEnter={() => this.popupTask(id)}
          onMouseLeave={() => this.popdownTask(id)}
          ref={(r) => { this.ref_thumbnail[id] = r; }}
          >
          <div className='rwm_thumbnail_preview'
            ref={(r) => { this.ref_thumbnails[id] = r; }}
            onClick={() => this.normalize(id)}
          >
            <canvas ref={(r) => { this.ref_thumbnailPreviewCanvas[id] = r; }} />
          </div>
          
          <ContextMenuTrigger id={'rwm_taskBarContext_' + id}
            holdToDisplay={-1}>
            <div className='rwm_thumbnail_bar'
              ref={(r) => { this.ref_thumbnailBar[id] = r; }}
              onClick={() => this.normalize(id)}
            >
              <div>{manager.wms[this.wmId].windows[w].title}</div>
              <div className='rwm_thumbnail_bar_state'
                ref={(r) => { this.ref_thumbnailBarState[id] = r; }}
              ></div>
            </div>
          </ContextMenuTrigger>
          <div>
            <ContextMenu id={'rwm_taskBarContext_' + id}>
              <MenuItem data={{ action: 'center' }} onClick={(e,data) => { this.handleContextMenuClick(e,data,id) } }>
                Center
              </MenuItem>
              <MenuItem data={{ action: 'unsnap' }} onClick={(e,data) => { this.handleContextMenuClick(e,data,id) } }>
                Unsnap
              </MenuItem>
              <MenuItem data={{ action: 'maximize' }} onClick={(e,data) => { this.handleContextMenuClick(e,data,id) } }>
                Maximize
              </MenuItem>
              <MenuItem data={{ action: 'minimize' }} onClick={(e,data) => { this.handleContextMenuClick(e,data,id) } }>
                Minimize
              </MenuItem>
              <MenuItem data={{ action: 'close' }} onClick={(e,data) => { this.handleContextMenuClick(e,data,id) } }>
                Close
              </MenuItem>
            </ContextMenu>
          </div>
