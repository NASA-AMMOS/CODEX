...
class WebGLHeat extends Component {
  ...
  sortRows() {
    ...
    let mr;
    for( let r = 1; r < this.dataLength; r++ ) { //each row
      countedIntensities[r] = 0;
      for( let v = 0; v < this.dataWidth; v++ ) { //each value
    ...
    for( let i = 1; i < countedIntensities.length; i++ ) { //1 because index 0 of countedInt is empty
  ...
  hideEmptyRows( hide, mask ) {
    if( hide ) {
      ...
      this.fullRowMapping = Object.assign( [], this.rowMapping );
      if( emptyRows !== undefined ) { //Perhaps we've no empty rows
        //Now strip those 0 hit rows from our rowMapping
        for( let i = this.rowMapping.length - 1; i >=0; i-- ){
          if( emptyRows.indexOf( this.rowMapping[i] ) !== -1 )
            this.rowMapping.splice( i, 1 );
        }
      }
      //Now recalculate so our new row map goes into effect
      this.recalculateMasks();
  ...
  sortRowsOLD() {
    ...
    for( let r = 0; r < this.dataWidth; r++ ) { //Each row
      let rm = this.featureMap( r );
      let sortMap = formulas.getSortMap( Object.assign( [], this.masks[this.sortMask].featureIntensitiesByRow[rm] ) ).sortIndices;
      
      for( let m in this.masks ) {
        let rowPts = [];
        for( let sm = 0; sm < sortMap.length; sm++ ) { //Each bin
          let curY = 0;
          for( let i = 0; i < this.featureHeight / this.binSize; i++ ) { //Each bin height
            rowPts.push( { x: sm * this.binSize, y: curY + (rm*(this.featureHeight+this.featureGap)),
              size: this.binSize, intensity: this.masks[m].featureIntensitiesByRow[rm][sortMap[sm]] } );
            curY += this.binSize;
          }
        }
      ...
      for( let r = 1; r < this.props.data.length; r++ ) {
        if( maskObj.masker( this.props.data.getIn([r, rowNumber]) ) ) {
          if( fullData.hasOwnProperty(this.props.data.getIn([r, rowNumber]) ) )
            ...
            fullData[this.props.data.getIn([r, rowNumber])] = 0;
        }
      }
    }
    else {
    ...
    if( this.props.data.has(rowNumber) ) {
      ...
      for( let r = 0; r < this.props.data.get(rowNumber).length; r++ ) {
        ...
        for( let p = 0; p < this.pixelBinSize; p++ ) {
          if( this.props.data.get(rowNumber + p) ) { //if the data doesn't fit neatly into a bin
            mr = this.masks[mask].masker( this.props.data.getIn([rowNumber + p, rm]), rm );
            if( mr > 0 ) {
              v += mr / this.pixelBinSize;
              if( !this.initialDrawHappened )
                this.masks[mask].rowHits[rm]++;
        ...
        for( let i = 0; i < this.featureHeight / this.binSize; i++ ) {
          rowPts.push( { x: binNumber * this.binSize, y: curY, size: this.binSize, intensity: intensity } );
          curY += this.binSize;
        }
        for( let i = 0; i < this.featureGap / this.binSize; i++ ) {
          rowPts.push( { x: binNumber * this.binSize, y: curY, size: this.binSize, intensity: 100000000 } );
          curY += this.binSize;
        }
      }
    }
  ...
  getMaskAtRowByRow( binNumber, mask ) {
    ...
    if( this.props.data.has(rowNumber) ) {
      ...
      for( let p = 0; p < this.pixelBinSize; p++ ) {
        ...
        if( this.props.data.has(vm) ) { //if the data doesn't fit neatly into a bin
      ...
      this.columnIntensity[rm] += ( intensity / this.dataLength );
      for( let i = 0; i < this.featureHeight / this.binSize; i++ ) {
        rowPts.push( { x: binNumber * this.binSize, y: curY + (r*(this.featureHeight+this.featureGap)), size: this.binSize, intensity: intensity/2 } );
        curY += this.binSize;
      }
      for( let i = 0; i < this.featureGap / this.binSize; i++ ) {
        rowPts.push( { x: binNumber * this.binSize, y: curY + (r*(this.featureHeight+this.featureGap)), size: this.binSize, intensity: 100000000 } );
        curY += this.binSize;
      }
    }
    return rowPts;
      ...
      if( this.currentBin < this.width ) {
        for( let m in this.masks ) {
          if( this.masks[m].on ) {
            ...
            this.masks[m].heatmaps[this.masks[m].h].addPoints( this.getMaskAtRowByRow( this.currentBin, m ) );
          }
        }
        this.currentBin++;
      }
  ...
  wheel( e ) {
    e.preventDefault();
    let windowPos = manager.getWindowPosition( this.props.rwm.windowId, this.props.rwm.managerId );
    ...
    var leftness = parseInt( this.masks[0].canvases[this.masks[0].h].style.left.replace('px',''), 10 ) || 0;
    var x = e.pageX - pos.x - windowPos.x - leftness;
    var y = e.pageY - pos.y;
    
    if( e.deltaY < 0 ) { //zoom up/in
      ...
      */
      ...
      this.oldPixelBinSize = this.pixelBinSize;
      this.pixelBinSize = Math.ceil( ( this.dataLength ) / ( this.width * Math.pow( 2, this.zoomLevel ) ) );
      //Without innerOffset, startRow will start at where the cursor zoomed in
      // We want it to zoom in directly to the cursor
  ...
  switchH() {
    ...
    for( let m in this.masks ) {
      ...
      this.masks[m].canvases[this.masks[m].h].style.zIndex = parseInt( this.masks[m].canvases[this.masks[m].h].style.zIndex, 10 ) + 2;
    }
  }
  ...
  mouseMove( e ) {
    if( this.mouseIsDown ) {
      let leftness = parseInt( this.masks[0].canvases[this.masks[0].h].style.left.replace('px',''), 10 ) || 0;
  ...
  mouseUp() {
    if( this.mouseIsDown ) {
      for( let m in this.masks ) {
        this.masks[m].canvases[this.masks[m].h].style.transition = 'opacity 1s cubic-bezier(0.445, 0.05, 0.55, 0.95)';
      }
      this.mouseIsDown = false;
    ...
    try {
      for( let m in this.masks ) {
        this.masks[m].heatmaps[0] = webglheatmap.createWebGLHeatmap(
          { canvas: this.masks[m].canvases[0], width: this.width, height: this.height, intensityToAlpha: false, gradientTexture: this.masks[m].gradient, gradientWidth: 340 }
        );
        /*
  ...
  mouseMoveBin(e) {
    ...
    let yBin = Math.floor( y / ( this.featureHeight + this.featureGap ) ) * ( this.featureHeight + this.featureGap );
  ...
  mouseClickValues( e ) {
    ...
    this.props.setInfoTable( this.props.data.getIn([0, yIM]) + ' - Row: ' + xIM + '+' + this.pixelBinSize, values );
  }
  ...
  _makeHeatmapCanvases() {
    ...
    for( let m = Object.keys( this.masks ).length - 1; m >= 0; m-- ) {
      canvases.push(
        <canvas ref={(r) => { this.masks[m].canvases[0] = r; } }
          key={m*2}
          style={{zIndex: 2}}
  ...
  render() {
    return (
      <div className='WebGLHeat' ref={(r) => { this.ref = r; }}
        onClick={(e) => { this.mouseClickValues( e ); } }
        onMouseMove={(e) => { this.mouseMoveBin( e ); } }
        onMouseLeave={(e) => { this.mouseLeaveBin( e ); } }>
        {this._makeHeatmapCanvases()}
        <div id='hoverBin' ref={(r) => { this.ref_hoverBin = r; } }></div>
      </div>
    );
  }
}
