...
class HoverableFS extends Component {
    constructor(props) {
        ...
        this.chosenSelections = getActiveSelectionNames(this.props.data)
            .map(s => s.get("name"))
            .toJS();
    ...
    componentDidUpdate() {
        ...
        this.chosenSelections = getActiveSelectionNames(this.props.data)
            .map(s => s.get("name"))
            .toJS();
        if (this.uglytempflag) this.forceUpdate();
        this.uglytempflag = false;
    }
                ...
                <div className="selectedFS">
                    <div className="selectedF">
                        <div className="selectedHeader">
                            <span>{this.chosenFeatures.length}</span> <span>{featureString}</span>
                        </div>
                        <ul
                            className="selectedCollapse"
                            ref={r => {
                                this.refF = r;
                            }}
                        >
                    ...
                    <div className="selectedS">
                        <div className="selectedHeader">
                            <span>{this.chosenSelections.length}</span>{" "}
                            <span>{selectionString}</span>
                        </div>
                        <ul
                            className="selectedCollapse"
                            ref={r => {
