...
def toseconds(humantimestring):
    ...
    return float(mlib_regex.extract_groups("([0-9.+]+)",humantimestring)[0]) * multiplier
...
def prettyfraction(val1,val2,decimals=0):
    ...
    return SI(val1,decimals = decimals)+" / "+SI(val2,decimals = decimals)+" = %0.1f%%"%(val1/float(val2+1e-99)*100)
...
def fixstr(strval, length, justify = 'left'):
    ...
    if   ('left'  in just) or ('lft' in just) or ('lt' in just) or ("l" in just):
        ...
        print "WARNING: mlib_formatting.fixstr invalid justification requested",justify
        return "?"*length
