...
def mvlr_lasso(X, Y, alpha = 1.0, fit_intercept = True, normalize = False, copy_X = True, precompute = True, max_iter = -1,
    ...
    >>> coefficients
    array([ 1.99744442,  4.95566575,  0.        ])

    >>> intercept
    10.033667251022621

    >>> rms
    0.022104577151410287
    ...
    if X.shape[0] != len(Y):
        raise Exception('Must provide regression target each sample, dimension mismatch between X %s & Y %s'%(str(X.shape),str(len(Y))))

    #Perform fit
    regressor = Lasso(alpha = alpha, fit_intercept = fit_intercept, normalize = normalize, copy_X = copy_X, precompute = precompute,
