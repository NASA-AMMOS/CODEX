...
def find_coastal_small_areas_chunk(lon_lnd, lat_lnd, time_lnd, lon_sea, lat_sea, time_sea, max_distance_km, min_soundings_to_consider_sa = 4, make_graphs = False, progressbar = False, sample_rate = 1):
    ...
    for chunk_range_i in lister:
        ...
        #Get the associated window of time_sea that corresponds to this land chunk, plus & minus half the time gap definition
        ...
        (chunk_SA_lnd,
         chunk_SA_sea) = find_coastal_small_areas(lon_lnd[chunk_indices_lnd], lat_lnd[chunk_indices_lnd], time_lnd[chunk_indices_lnd],
        ...
        #Have to increase SA id's as we append them to the larger, non-chunked set, but only if they're not -1 special value
...
#Find coastal small areas in order of the highest data density
#per Coastal Small Area. Remove these points from consideration
#and continue looking for additional small areas until we can't find something
#suitable. Considers all LND points as centers for Coastal Small Areas, as there's many less of these than ocean in general
#Sample_rate shows how many sounding id's should be skipped in initial SA size estimate (1 = consider everything, 2 = skip every other).
#this is due to the extreme cost of evaluating every indices in a large dataset. This will
#make the SA's chosen not quite optimal. However, metrics show little actual difference and massive speedup.
#assume_time_order causes the function to skip ordering by time, assuming all is already well.
...
#        print "Curtime, lon, lat",curtime,curlon,curlat
        ...
        #only examine remotely possible times, to avoid needless computation
        #Note that anywhere that is not in the available mask has 5e9 seconds subtracted from its time, removing it from consideration
        #This takes all masking into consideration in returned results
...
#    print "Sorting"
    ...
    potential_SA_sea_indices= [None,]*len(indices_lnd_sampled)  #This has size of indices_lnd because that's how many "arrangements" we're considering
    first_loop = True

    if progressbar:
        #progressbar is based on the mask of what soundings are still available, so bar is a tad longer than it should be
        bar = mlib_progressbar.ProgressBar(N.sum(mask_lnd_available))
        bar.start()
    ...
    while last_SA_count >= min_soundings_to_consider_sa:
        ...
        #But only do it every once in awhile or it overwhelms the log file with millions of progress bar images for Gigs of crap
        ...
        mask_update_sampled = (time_lnd_sampled > lo_time - max_duration_seconds_sa) & (time_lnd_sampled < hi_time + max_duration_seconds_sa)

        #potential_SAs is an array of (SA_size, (indices used)). Complex, and not an Ndarray. So can't MASK into it, but can slice
        if progressbar and first_loop:
            print "Updating potential sizes of SA's"
            bar2 = mlib_progressbar.ProgressBar(N.sum(mask_update_sampled))
            bar2.start()
        ...
        for c,i in enumerate(N.where(mask_update_sampled)[0]):
            ...
            potential_SA_lnd_indices[i] = sizes_and_indices[c][1] #These indices lists are NOT sampled but whole array relevant
            potential_SA_sea_indices[i] = sizes_and_indices[c][2] #These indices lists are NOT sampled but whole array relevant

        if make_graphs:
            print mlib_log.log_time_and_mem(T, "Update size graph", SA_cur)
...
#            print "Existing mask_available:",mask_available
        ...
        if make_graphs:
            ...
            P.hold(True)
            P.plot(range(len(potential_SA_counts)),potential_SA_counts,'r.',alpha=0.5,markersize=2,label='after selection')
            P.xlabel('indices')
...
#This wrapper chunks a time record by day, so the quadratic algorithm to find small areas doesn't go crazy
def find_small_areas_chunk(lon, lat, time, max_distance_km, min_soundings_to_consider_sa = 4, max_time_gap_seconds = 20*60,
...
#Find small areas in a single dataset in order of the highest data density
#per Small Area specification is found. Remove these points from consideration
#and continue looking for additional small areas until we can't find something
#suitable.
#Sample_rate shows how many sounding id's should be skipped in initial SA size estimate (1 = consider everything, 2 = skip every other).
#this is due to the extreme cost of evaluating every indices in a large dataset. This will
...
#    print "Sorting"
    ...
    if progressbar:
        #progressbar is based on the mask of what soundings are still available, so bar is a tad longer than it should be
        bar = mlib_progressbar.ProgressBar(N.sum(mask_available))
        bar.start()
    ...
    while last_SA_count >= min_soundings_to_consider_sa:
        ...
        mask_update_sampled = (time_sampled > lo_time - max_duration_seconds_sa) & (time_sampled < hi_time + max_duration_seconds_sa)
        ...
        #Assumes consecutive, but time-sorting guarentees that
        for c,i in enumerate(N.where(mask_update_sampled)[0]):
            potential_SA_indices[i] = sizes_and_indices[c][1] #These indice lists are NOT sampled but whole array relevant
...
def ensure_integer_full_sounding_ids (sounding_ids):
    ...
    if not mlib_types.isint(sounding_ids[0]):
        if mlib_types.isstr(sounding_ids[0]):
...
# #-------------------------------
# def mask_following_footsteps_notwork (lons, lats, sounding_ids, max_angle_degrees = 4, min_footprints_per_frame = 6, ground_track_secs = 600.0, progressbar = False, n_jobs = 10):
#     """Find soundings whose frames are aligned with the spacecraft's ground track.
#     I call these following footsteps; OCO-2 calibration calls them Streak Fleats (ugh!)

#     Args:
#         lons                    : iterable of longitudes
#         lats                    : iterable of latitudes
#         sounding_ids            : sounding ids (OCO-2 style) of the form YYYYMMDDHHMMSSRF where R is fractional second and F is footprint # 1-8
#         max_angle_degrees       : The maximum difference between frame angle and ground track heading to determine following_footstep
#         ground_track_secs       : How far back in time to look at frames to determine ground track vector
#         min_footprints_per_frame: Frames with less than this number of member footprints will not be selected
#         progressbar             : Display a progress bar for user feedback
#         n_jobs                  : The number of jobs to spawn, -1 equals the number of cores, 1 means single thread, default 10
...
#     frames, matching_indices, footprints, lons, lats = frames[mask], matching_indices[mask], footprints[mask], lons[mask], lats[mask]
...
#     footprint_bearing_per_frame = [ calculate_compass_bearing ( ( _lats[0], _lons[0] ), ( _lats[-1], _lons[-1] ) ) for _lats, _lons in zip(lats_per_frame, lons_per_frame) ]
...
#                                                        footprint_bearing_per_frame[_frame]) ) < max_angle_degrees for _frame in frames ]
...
#     max_dist = mlib_latlon.latlon_to_km_distance_NS_EW(max_lon_at_equator, max_lat_at_equator, 0.0, 0.0)

#     for i in range(num1):
#        dist_lon, dist_lat = mlib_latlon.latlon_to_km_distance_NS_EW(lon1[i           ], lat1[i           ],
...
def mask_mutually_nearby_data_spacetime(lon1, lat1, time1, lon2, lat2, time2, max_dist_meters, max_time, return_nearest_indices = False):
...
#    mask21 = dists21 < max_dist_meters
    #Now use indices21 to check for TWO independent conditions in space AND time, not the combined metric used to find nearest neighbor
    print "   Applying hard cutoff matching filter to nearest neighbors"
    mask21 = (
        ( (time2 - time1[indices21]) <= max_time ) &
        ( N.sqrt( (x2 - x1[indices21])**2 + (y2 - y1[indices21])**2 + (z2 - z1[indices21])**2 ) < max_dist_meters)
        )
...
#    mask12 = dists12 < max_dist_meters
    ...
    mask12 = (
        ...
        ( N.sqrt( (x1 - x2[indices12])**2 + (y1 - y2[indices12])**2 + (z1 - z2[indices12])**2 ) < max_dist_meters)
        )
...
def mask_mutually_nearby_data(lon1, lat1, lon2, lat2, max_dist_meters):
    ...
    if len(lon1) != len(lat1): raise Exception('Lon and Lat for population 1 must be equal length %d vs %d'%( len(lon1), len(lat1) ) )
    if len(lon2) != len(lat2): raise Exception('Lon and Lat for population 2 must be equal length %d vs %d'%( len(lon2), len(lat2) ) )
...
def mask_mutually_nearby_data_1D(var1, var2, max_dist):
    ...
    if not mlib_types.isnarray(var1): var1 = N.array(var1)
    if not mlib_types.isnarray(var2): var2 = N.array(var2)
...
def bounding_ellipse(center_lat, center_lon, lat_radius, lon_radius, lat_array, lon_array):
    ...
    return (N.array(lat_array) - center_lat)**2 + (delta_angle(lon_array, center_lon)*lat_radius/float(lon_radius))**2 <= lat_radius*lat_radius

# if __name__ == "__main__":


#    print "\nChecking directional angles"
#    for pointset in [ ( (0,0),( 10,10) ) , ( (0,0),(-10,-10) ) ,
#                      ( (0,0),(-10,10) ) , ( (0,0),( 10,-10) ) ]:
#       latlon1 = pointset[0]
#       latlon2 = pointset[1]
#       print "angular_heading  :",latlon1, latlon2, "%0.2f"%angular_heading  (latlon1, latlon2),"degrees"
#       print "angular_alignment:",latlon1, latlon2, "%0.2f"%angular_alignment((latlon1[0],latlon2[0]),(latlon1[1],latlon2[1])),"degrees"

#    print "\nChecking lat/lon relations"
#    for pointset in [ ( (0,0),( 10, 5) ) , ( (0,0),(  5,10) )  ]:
#       latlon1 = pointset[0]
#       latlon2 = pointset[1]
#       print "angular_heading  :",latlon1, latlon2, "%0.2f"%angular_heading  (latlon1, latlon2),"degrees"
#       print "angular_alignment:",latlon1, latlon2, "%0.2f"%angular_alignment((latlon1[0],latlon2[0]),(latlon1[1],latlon2[1])),"degrees"

#    print "\nChecking extreme angles"
#    for pointset in [ ( (0,0),( 10,0 ) ) , ( (0,0),(  0, 10) ) ,
#                      ( (0,0),(-10,0 ) ) , ( (0,0),(  0,-10) ) ]:
#       latlon1 = pointset[0]
#       latlon2 = pointset[1]
#       print "angular_heading  :",latlon1, latlon2, "%0.2f"%angular_heading  (latlon1, latlon2),"degrees"
