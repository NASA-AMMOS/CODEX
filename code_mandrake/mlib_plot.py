...
def init(backend = None, interactive = False):
    ...
    Invoke during import command to mlib_plot as:
         import mlib_plot as MP; MP.init(interactive = True)
         import mlib_plot as MP; MP.init() #for no-term requirement png renderer
    ...
    if (backend is None): backend = 'TkAgg' if interactive else 'Agg'
...
def plot_relation_circle(labels, connections, connection_indices = None, separations = None,
    ...
    Args:
        ...
        seperations  : Indices into labels that should be shown as a wider gap for grouping
    ...
    if connections.ndim == 1:
        if connection_indices is None:
            raise Exception("Must provide either 2D connections matrix, or 1D list along with (1D,1D) connection_indices")
        else:
...
def label_decimal_years(index_year = 2015):
    ...
    for year in years_to_consider:
        for month in mlib_time.MONTHS_BY_DOY:
            x = year + mlib_time.MONTHS_BY_DOY[month][0]/365.0
            if lx <= x <= rx:
...
def global_scatterplot_multiseries(x_pop_list, y_pop_list, title="", xlabel="", ylabel="",
                                   filename = None, palette = "rainbow_black_red", projection='cyl', limits = [-180, 180, -90, 90],
                                   colors = None, labels = None, markersize = 5, markeralpha = 1.0, markertype = '.',
                                   legend = True, marginal_bins = 25, legend_alpha = 0.9, legend_loc = 'best', legend_font = 'small', dpi=150,
                                   resolution = 'l', background = None, samefig = False, legend_markersize = None,
                                   grid_dashes = DEFAULT_DASHES, grid_alpha = 0.5, linewidth = None, linestyle = '-', linealpha = None,
                                   ):
    ...
    map.drawmeridians(meridians, labels=[0,0,0,1], color=(0.0,0.0,0.0), fontsize=5, alpha = grid_alpha, dashes = grid_dashes)
    map.drawparallels(parallels, labels=[1,0,0,0], color=(0.0,0.0,0.0), fontsize=5, alpha = grid_alpha, dashes = grid_dashes)
    ...
    if legend and numpops > 1: nicelegend(framealpha = legend_alpha, loc = legend_loc, markersize = legend_markersize, size = legend_font)
...
#-------------------------------------
def global_scatterplot_groups(x, y, groups = None, z = None, func = None, title="", xlabel="", ylabel="", clabel="", filename = None, palette = "gist_rainbow_r", projection='cyl', limits = [-180, 180, -90, 90], markersize = 5, markertype = '.', colorbar = True, dpi = 150, resolution = 'l', background = "", zlimits = None, samefig = False, grid_dashes = DEFAULT_DASHES, grid_alpha = 0.5):
    ...
    sp = P.scatter(x, y, s = markersize, marker = markertype, c = colors, alpha = 1, cmap = C.custom_colormap(palette), lw=0, vmin=zlimits[0], vmax=zlimits[1])
...
def plot_heatmap_square (x, y, z = None, log = False, bins = 50, limits = None, func = "count",
    ...
    x_bins, y_bins, z_grid = NUM.binned_stat_2d(x, y, z = z, func = function, binsx = binx, binsy = biny, limitsx = limits[0:2], limitsy = limits[2:4])
    ...
    if mlib_types.isbool(fit_line) and fit_line:
...
#        P.plot(x, ypredict, '-b', alpha=0.3, linewidth=4)
        ...
        if show_equation:
            ...
            if ("both" in equation_style.lower()) or ("intercept" in equation_style.lower()):
                eqtext.append(mlib_equation.text_equation_simple_linear_fit( m, b, style = 'intercept' ) )
            if ("both" in equation_style.lower()) or ("meaned" in equation_style.lower()):
                eqtext.append(mlib_equation.text_equation_simple_linear_fit( m, b, style = 'mean'      ) )
            eqtext = "\n".join(eqtext)+"\nR2 %0.2f"%R2_adj
    ...
    if is_iterable(fit_line):
        ...
        if show_equation:
            ...
            for i in range(len(coef_arr)):
                ...
                if ("both" in equation_style.lower()) or ("meaned" in equation_style.lower()):
                    eqtext.append("%s = %0.2g*(%s + %0.2g)" % ( 'Y', coef_arr[i][0], 'X', coef_arr[i][1]/coef_arr[i][0] ))
            eqtext = "\n".join(eqtext)
...
def plot_histogram_multiseries(val_list, weights = None, filename = None, bins = 80,
    ...
    for i, (series, weights) in enumerate(zip(data_list, weights)):
        ...
        if len(series) > 0:
            bins, counts = NUM.binned_stat(series, y=weights, bins = bins, limits = limits, func = func, ignore_outliers = ignore_outliers)
        else:
            continue
    ...
    markplot()
    disable_axis_offset()
    if not yaxis: visible_axes(['bottom',])
    else        : visible_axes()
...
def global_heatmap_square(lons, lats, z = None, title="", xlabel="", ylabel="", filename = None,
    ...
    lon_array, lat_array, z_grid = NUM.binned_stat_2d(lons, lats, z = z, func = func, binsx = binx, binsy = biny, limitsx = limits[0:2], limitsy = limits[2:4])
    #Bin the data into a count grid
    lon_array, lat_array, count_grid = NUM.binned_stat_2d(lons, lats, binsx = binx, binsy = biny, limitsx = limits[0:2], limitsy = limits[2:4])
...
def global_plot_grid(lons, lats, z,
                     ...
                     label_lon = False, label_lat = True, colorbar = True, dpi=150, grid_dashes = DEFAULT_DASHES, grid_alpha = 0.5):
...
#    map.fillcontinents(color='grey')
    ...
    map.drawmeridians(lons - delta_lon / 2.0, labels=labels, linewidth=0.5, color=(0.0,0.0,0.0), fontsize=5, alpha = grid_alpha, dashes = grid_dashes)
    labels = [1,0,1,1] if label_lat else [0,0,0,0]
    delta_lat = N.abs(lats[1]-lats[0])
    map.drawparallels(lats - delta_lat / 2.0, labels=labels, linewidth=0.5, color=(0.0,0.0,0.0), fontsize=5, alpha = grid_alpha, dashes = grid_dashes)
    ...
    if contour:
        ...
        if colorbar:
            ...
            cb = nicecolorbar(CS2, source = map, location = 'bottom', clabel = clabel, ticks = significant_ticks(vmin,vmax,10))
...
def polygon (x, y, color='k',alpha=0.1, label = ""):
   ...
   poly = Polygon(zip(x,y),fill=True,fc=color,ec=color,lw=0.0, alpha=alpha,edgecolor = 'none', visible=True, label=label)
...
def plot_varyregion (x, y_lower = None, y_upper = None, y = None, widths = None, colors='k', alphas=0.1, label = ""):
    ...
    for i in range(numpoints-1):
        ...
        #Make sure only one segment is labelled, but choose one from ~the center instead of zero index. More representative?
...
def plot_hex_map(lat, lon, data, func, filename, clabel = None, MIN=None, MAX=None):
    ...
    map.drawmapboundary()
    x, y = map(lon, lat)
    im = map.hexbin(x, y, C = data, reduce_C_function = func, gridsize = 72, cmap = P.get_cmap('gist_rainbow_r'),vmin=MIN,vmax=MAX)
#    formatter = matplotlib.ticker.ScalarFormatter( useOffset = False )
...
#This is an improved legend function that defaults to nice-looking output and can receive a
#"size" keyword just like the P.txt function instead of a horrible fontproperties object.
#It does not receive labels or handles directly; it assumes these have been specified already
#in appropriate plotting function calls using "label=" notation.
#Valid sizes are (size in points), xx-small, x-small, small, medium, large, x-large, xx-large
#Also adjusts words like "middle" to "center" for compatability with loc argument
#Using a framealpha will show the underlying shadow, so the code automatically turns shadow off with any transparency.
#numpoints specifies how many points to include. Default is two if you have lines shown, often not what you want. We set default to 1
#markersize chanegs the size of the shown markers in the legend for clarity, useful if the graph is plotting very small markers
def nicelegend ( loc = 'best', borderpad = 0.1, size = 'x-small', framealpha = 1.0, fancybox = True, shadow = False, numpoints = 1, handles = None, labels = None, markersize = None, facecolor = (1,1,1)):
...
#    mapi.fillcontinents(color='grey')
    ...
    parallels = ticks_globe_degrees(limits[2], limits[3])
    labels = [1,0,1,1] if label_lon else [0,0,0,0]
    mapi.drawmeridians(meridians,labels=labels, color=(0.0,0.0,0.0), fontsize=5, alpha = grid_alpha, dashes = grid_dashes)
    labels = [1,0,1,1] if label_lat else [0,0,0,0]
    mapi.drawparallels(parallels,labels=labels, color=(0.0,0.0,0.0), fontsize=5, alpha = grid_alpha, dashes = grid_dashes)
...
def barchart_multiseries(yval_pop_list, group_names_pop_list = None, title="", ylabel="", filename = None,
    ...
    for pop in range(numpops):
        ...
        rects.append(P.bar(bar_centers, yval_pop_list[pop], barwidth, color=colors[pop], yerr=yerr_pop_list[pop], ecolor = 'k'))
        ...
        if show_bar_values:
            for rect in rects[-1]:
                ...
                if showheight < 2:
                    ...
                    format = "%d"
                P.text(rect.get_x() + rect.get_width()/2.0, 1.05*height, format%showheight, size = 'xx-small', ha='center',va='bottom')

    if legend and numpops > 1:
        nicelegend(handles = [rects[i][0] for i in range(numpops)], labels = labels, framealpha = legend_alpha, loc = legend_loc)
...
#-----------------------
def plot_confusion_matrix(confusion_matrix, filename = None, labels = None, title='Confusion matrix', cmap='afmhot', samefig = False):
    """Plots a traditional confusion matrix with 0-based integer class assignments.

    Args:
        confusion_matrix: An NxN array produced by mlib_classification.confusion_matrix (truth is first index, pred is second)
...
def plot_distribution_matrix(distribution_matrix, bins, norm_each = False, legend = False,
    ...
    >>> from mlib_classification import distribution_matrix
    >>> N.random.seed(0)
