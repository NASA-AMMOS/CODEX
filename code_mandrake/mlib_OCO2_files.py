...
class L2(OCO2_datafile):
    ...
    def read(self, action, paths, allowed_modes = ('nadir','glint','target','transition'), progressbar = False ):
        ...
        for fi,filer in enumerate(sorted(self.paths_input)):
            ...
            #Immediately process the acquisition mode, as it might omit this entire file off the bat
            ...
            numpoints = len(thisdata["L2/RetrievalGeometry/retrieval_latitude"])
...
#            thisdata['L2/Metadata/AcquisitionMode'] = N.ones(numpoints, dtype=N.int8) * mode_types[ h['Metadata']['AcquisitionMode'][0].lower() ]
...
#                thisdata['Mandrake/expform_airmass_B_%03.1f'%B] = N.exp(B*(thisdata['Mandrake/airmass']-2.0))-1.0
            ...
            for typer in aerosol_types:
                ...
                istype0 = [typer in x[0] for x in h['AerosolResults']['aerosol_types'][:]]
