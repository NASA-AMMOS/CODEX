...
def map_norm_rgb_to_hex(norm_rgb):
    ...
    #Remove alphas
    if len(norm_rgb) > 3: norm_rgb = norm_rgb[:3]

    hexvals = [hex(int(round(x*255)))[2:] for x in norm_rgb]
...
def custom_colormap(name):
    ...
    if isinstance(name, C.LinearSegmentedColormap): return name
...
def color_span(number_elements, palette = None):
    ...
    array([[ 0.5       ,  0.        ,  1.        ,  1.        ],
           [ 0.16662598,  0.86621094,  0.86621094,  1.        ],
           [ 0.83349609,  0.86621094,  0.5       ,  1.        ],
...
def map_values_to_rgba(values, palette = None, scale = False, uint8 = False):
    ...
    >>> PP(map_values_to_rgba(N.linspace(0,1,10), palette = 'spectral'))
    [(0.0, 0.0, 0.0, 1.0),
    ...
    >>> PP(map_values_to_rgba(N.linspace(0,0.5,10), palette = 'spectral'))
    [(0.0, 0.0, 0.0, 1.0),
    ...
    >>> PP(map_values_to_rgba(N.linspace(0,0.5,10), palette = 'spectral', scale = True))
    [(0.0, 0.0, 0.0, 1.0),
     (0.42873137254901961, 0.0, 0.61307843137254903, 1.0),
     (0.0, 0.18301960784313726, 0.86670000000000003, 1.0),
     (0.0, 0.64446666666666663, 0.73336666666666672, 1.0),
    ...
    if palette is None: palette = P.cm.cmapname
