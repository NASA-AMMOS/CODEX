...
def enforce_literal(string_array):
   ...
   if was_singleton: string_array = [string_array,]
   retvals = []

   for string in string_array:
      #'\\' must come first or else will match backslashes added by later chars
      for char in SPECIAL_CHARS: string = string.replace(char,'\\'+char)
      retvals.append(string)
   if was_singleton: retvals = retvals[0]
   return retvals
...
def extract_groups(patternstring, stringlist, results_per_string = False):
    ...
    if len(results) == 0 and singleton: return None

    #Handle singleton result
    if singleton:
       if len(results) == 1 and not results_per_string: return results[0]
       else                                           : return results
...
def matching_elements(patternlist, stringlist, unique = True, sort = False, unmatched = False, indices = False,
   ...
   if is_not_iterable(patternlist): patternlist = [patternlist,]
   if is_not_iterable(stringlist ): stringlist  = [stringlist ,]
   if len(patternlist) == 0: return []

   if force_literal: patternlist = [enforce_literal(x) for x in patternlist]
...
def keep_these_chars(stringlist, chars_to_keep = None, set_to_keep = None, ignore_case  = False):
   ...
   if chars_to_keep is None: chars_to_keep = ""
   if set_to_keep is not None: chars_to_keep += charset[set_to_keep.lower()]
   if ignore_case: chars_to_keep = chars_to_keep.lower() + chars_to_keep.upper()

   table_all  = string.maketrans('','')
   table_filt = table_all.translate(table_all, chars_to_keep)

   if is_not_iterable(stringlist): return stringlist.translate(table_all, table_filt)
   return [x.translate(table_all, table_filt) for x in stringlist]
