...
def glob(globspec):
    ...
    >>> glob([repo_path() + '/mlib/s*.py', repo_path() + '/mlib/co*.py']) # doctest: +ELLIPSIS
    ['.../mlib/shell.py', '.../mlib/spanning.py', '.../mlib/color.py', '.../mlib/correlation.py']
...
def make_archive(source, destination):
    shutil.make_archive('_{:%Y-%m-%d}'.format(datetime.now()), "gztar", source, destination)
...
def head(filepath, N):
    ...
    try:
        with open(filepath) as f:
            return "".join([next(f) for x in range(N)])
