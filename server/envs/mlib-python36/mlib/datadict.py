...
# mlib.datadict.py: FutureWarning: in the future, boolean array-likes will be handled as a boolean array index
import warnings
...
def resolve_features(features_all, keys, multiple=False, literal=False, ignore_case=False, exists=False):
    ...
    >>> import mlib.HDF5 as H
    >>> testfile = doctest_input_path()+"/sample_oco2.h5"
...
class DataDict:
    ...
    def __init__(self, indict=None, usecols=None, deepcopy=False, mask=None, delimiter='\\s+', maxrows=None):
        ...
        Read in a csv file
        >>> testfile = doctest_input_path()+"/threshold_CMPLX_05_relative_residual_mean_square_weak_co2.txt"
        ...
        if indict is not None:
            ...
            if isinstance(indict, dict):
                ...
                for h in self.dictionary:
                    self.dictionary[h] = N.array([self.dictionary[h], ]).squeeze()
            ...
            elif mlib.mtypes.isstr(indict) or (mlib.mtypes.isarray(indict) and mlib.mtypes.isstr(indict[0])):
            ...
            # Pandas case, placed in else because we don't want to load Pandas unless we have to
                ...
                if isinstance(indict, DataFrame):
                    if usecols is None:
                        usecols = indict.columns.values.tolist()
                    self.features = usecols
                    self.dictionary = dict([(feat, N.array(indict[feat])) for feat in usecols])
                else:
                    raise Exception(
                        "Must pass dictionary or DataDict or Pandas.DataFrame to initialize DataDict, not " + str(
                            type(indict)))
        else:
            self.dictionary = {}
            self.features = []
    ...
    def resolve_features(self, keys, multiple=False, literal=False, ignore_case=False, exists=False):
        ...
        was_iterable = not isinstance(keys, str) and not isinstance(keys, int)

        # If asked for no keys as an interable array, return empty array
        if was_iterable and len(keys) == 0:
            return []

        # Handle boolean case, just masks feature names and returns
        if was_iterable and mlib.mtypes.isbool(keys[0]):
            if len(keys) != len(self.features):
                raise Exception("Boolean mask must match number of features present")
            if exists:
        ...
        for key in keys:
            ...
            if mlib.mtypes.isstr(key):
                ...
                if not literal:
                    matches = mlib.regex.matching_elements(key, self.features, ignore_case=ignore_case)
                else:
                    if ignore_case:
        ...
        if was_iterable or multiple:
            ...
            from mlib.numeric import unique
    ...
    def append(self, field, thing_to_add, literal=False):
        ...
        if I.is_not_iterable(thing_to_add):
            ...
            if not mlib.mtypes.isnarray(thing_to_add): thing_to_add = N.array(thing_to_add)
    ...
    def __setitem__(self, key, thing_to_set):
        ...
        if resolved_key is None and mlib.mtypes.isint(key):
    ...
    def numsamples_numfeatures(self):
        ...
        if numfeatures == 0:
            return 0, 0
        numsamples = self.dictionary[list(self.dictionary.keys())[0]].size
        return numsamples, numfeatures

    # -----------------------------------
    # synonym with numsamples_numfeatures
    def shape(self):
        return self.numsamples_numfeatures()
    ...
    def numsamples(self):
        return self.numsamples_numfeatures()[0]
    ...
    def isempty(self):
        return N.prod(self.numsamples_numfeatures()) == 0
    ...
    def read_csv(self, filename, usecols=None, maxrows=None, delimiter=None, skipfooter=0, mask=None):
        ...
        >>> testfile  = doctest_input_path()+"/threshold_CMPLX_05_relative_residual_mean_square_weak_co2.txt"
        >>> testfilec = doctest_input_path()+"/flatfile.csv"
        >>> data = DataDict()
        ...
        >>> wrongfile = doctest_input_path()+"/dogo_pareto_2016042703154300.txt"
        ...
        for filer in filename:
            ...
            if len(self.features) > 0:
                if (len(dataframe.columns.values.tolist()) != len(self.features)) or N.any(
                        N.array(dataframe.columns.values.tolist()) != N.array(self.features)):
    ...
    def read_h5(self, filename, usecols=None, maxrows=None, mask=None):
        ...
        >>> wrongfile = doctest_input_path()+"/svego_test.h5"
        ...
        for filer in filename:
            ...
            if len(self.features) > 0:
                if (len(data.features) != len(self.features)) or N.any(
                        N.array(data.features) != N.array(self.features)):
    ...
    def write_csv(self, filename, usecols=None):
        ...
        >>> import mlib.HDF5 as H
        >>> path = doctest_input_path()
        >>> outpath = doctest_output_path()
        >>> infile  = path+  "/sample_oco2.h5"
        >>> chkfile = path+  "/sample_oco2.txt"
        >>> outfile = outpath+"/sample_oco2.txt"
    ...
    def write_HDF(self, filename, usecols=None):
        ...
        >>> H.compare_files(outfile, infile)#,True)
        True
        """
        # print(self.features)
        if usecols is None:
    ...
    # ------------
    def create_subsample(self, num_desired=None, with_replacement=False, return_indices=False, preselect=None,
                         usecols=None,
        ...
        from mlib.numeric import subsample_indices
        ...
        if mask is not None:
            ...
            indices = subsample_indices(num_available=numrows, num_desired=num_desired,
    ...
    def stats(self, usecols=None):
        ...
        for feat in usecols:
            stats['std'][feat] = N.std(self.dictionary[feat].astype(N.float64),
                                       ddof=1)  # divide by sqrt(N-1) like the rest of humanity
    ...
    def normalize(self, kind="standardize", usecols=None, literal=False):
        ...
        from mlib.numeric import normalize
        for feat in usecols:
    ...
    def unchanging_features(self):
        ...
        Test string data
        >>> data = DataDict({'one':['tree','car','wash'], 'two':['tree','tree','tree'], 'three':range(10)})
        >>> data.unchanging_features()
        ['two']
        ...
        if self.numsamples() == 0: return []
        return [feat for feat in self.features if len(Counter(self.dictionary[feat]).keys()) < 2]
    ...
    def highly_correlated_features(self, number_to_compare=None):
        ...
        for ifeat, feat in enumerate(self.features[:-1]):
            # Only compare features to the right because you've already compared those to the left above
            for ifeat2, feat2 in enumerate(self.features[ifeat + 1:]):
                if N.abs(C.pearson_correlation(self.dictionary[feat][mask],
                                               self.dictionary[feat2][mask])) > CORRELATION_FOR_EQUALITY:
    ...
    def mask_matching_values(self, match_feat, match_values, assume_unique=True):
        ...
        idx_val, idx_dd = self.matching_indices(match_feat, match_values, assume_unique=assume_unique,
                                                omit_mismatch=True)
    ...
    def matching_indices(self, match_feat, match_values, assume_unique=True, omit_mismatch=False):
        ...
        if assume_unique:
            ...
            val_idx_for_each_dd = N.array([(lookup[x] if x in lookup else -1) for x in self.dictionary[match_feat]],
                                          dtype=int)
