...
#
#  mlib.mtime.py -- Date & Time class based on native Python datetime, time, and calendar
...
MONTHS_BY_DECIMAL_YEAR = dict(
    [(month, (MONTHS_BY_DOY[month][0] / 364.0, MONTHS_BY_DOY[month][1] / 364.0)) for month in MONTHS_BY_DOY])
...
def to_J2000(input, format=None, shift=0.0):
    """ Main routine that takes in nearly any time format and brings it to J2000 standard.
        ...
        Input YYMMDD
        >>> to_J2000("20040606", format='YYMMDD')
        139752000.0
        ...
        >>> to_J2000((2004, 0o06, 0o06, 0o01, 0o01, 01.11))
        139755661.11
    ...
    if format is None:
        ...
        format = format.upper()
    if format == "J1970":
        return convert_1970_to_J2000_epoch(input)
    if format == "MJD":
        return convert_MJD_to_J2000_epoch(input)
    # assume J2000 seconds for any bare number
    if mlib.mtypes.isint(input) or mlib.mtypes.isfloat(input) or format.lower() == 'j2000':
        return float(input)
    # if it's a list, simple... will be interpretted as y,m,d,hh,mm,ss with 0's in any unspecified slot
    elif mlib.mtypes.isarray(input):
        return list_to_J2000(input)
    # if it's a string, could be many things
    elif isinstance(input, str):
        if input.lower()[:3] == "now":
            # Could be NOW or NOWUTC or NOWLOCAL (same as NOW)
            adjustment_hours = R.extract_groups("([-+\d\.\s]+)", input.lower())  # +- is in HOURS
            adjustment_hours = 0.0 if adjustment_hours is None else float(adjustment_hours.replace(" ", ""))

            if "utc" in input.lower():
                return to_J2000(
                    time.gmtime()[0:6]) + adjustment_hours * 60.0 * 60.0 + shift  # shift is passed in through function
            else:
                return to_J2000(time.localtime()[
                                0:6]) + adjustment_hours * 60.0 * 60.0 + shift  # shift is passed in through function
        ...
        # extract out only the numbers, drop all other characters regardless of fanciness. It's all Y M D H M S F
        # info anyway
        justdigits = R.keep_these_chars(input, set_to_keep='digits')

        # Handle the formats that only permit YY
        if format in ('YMMDD', 'YYMMDDHHMMSS'):
            doubles = [_numlist_to_int(justdigits[i * 2:i * 2 + 2]) for i in range(len(justdigits) // 2)]
            # Change YY into YYYY
            doubles[0] = ensureYYYY(doubles[0])
            # Handle YYMMDD case
            if len(justdigits) == len('YYMMDD'):
                return list_to_J2000(doubles)
        ...
        if len(justdigits) > 8:
            return list_to_J2000((_numlist_to_int(justdigits[0: 4]),
                                  _numlist_to_int(justdigits[4: 6]),
                                  _numlist_to_int(justdigits[6: 8]),
                                  _numlist_to_int(justdigits[8:10]),
                                  _numlist_to_int(justdigits[10:12]),
                                  _numlist_to_int(justdigits[12:14])))\
                   + frac + float("0.{}".format(_numlist_to_int(justdigits[14:])))
        else:
            return list_to_J2000((_numlist_to_int(justdigits[0: 4]),
                                  _numlist_to_int(justdigits[4: 6]),
                                  _numlist_to_int(justdigits[6: 8])))
...
def _numlist_to_int(numlist):
    """
    Converts a list of numerics to an integer

    :param numlist: list of numerics
    :return: numlist as an int

    >>> _numlist_to_int([1, 2, 3])
    123
...
def from_J2000(sec=0, format="YYYYMMDD_HHMMSS", aux=None):
    ...
    if format == "YYYYMMDDHHMMSSFF": return "%04d%02d%02d%02d%02d%02d%02d" % (
        y, m, d, hh, mm, ss, 0 if len(f) == 0 else int(float(f) * 100))
...
class Mtime:
    ...
    def __init__(self, input=None, format=None, aux=None,
                 shift=0.0):  # input is the time, format is the time format, and aux is any auxilliary information that may be required by the format
        self.shift = shift
    ...
    def nowset(self,
               newnow):  # this sets an offset from clock NOW to pretend to be other datetime, will be subtracted
        # from all readouts
        self.shift = Mtime(newnow) - self.now() + 0.0
        return self

    def nowsetUTC(self,
                  newnow):  # this sets an offset from clock NOW UTC to pretend to be other datetime, will be
        # subtracted from all readouts
    ...
    def floor(self, interval):  # round current object to a specified accuracy
        ...
        if interval.find('year') >= 0:
            self.sec = to_J2000((y, 1, 0, 0, 0, 0))
        elif interval.find('month') >= 0:
            self.sec = to_J2000((y, m, 1, 0, 0, 0))
        elif interval.find('day') >= 0:
            self.sec = to_J2000((y, m, d, 0, 0, 0))
        elif interval.find('hour') >= 0:
            self.sec = to_J2000((y, m, d, hh, 0, 0))
        elif interval.find('minute') >= 0:
            self.sec = to_J2000((y, m, d, hh, mm, 0))
        elif interval.find('second') >= 0:
            self.sec = to_J2000((y, m, d, hh, mm, int(ss)))
        else:
            die("Mtime: Floor: Malformed interval: " + interval)
        return self
...
def J2000_to_decimal_year(J2000):
    ...
    for t2000 in J2000:
        ...
        temp = (t2000 - to_J2000(YYYY + "0101000000")) / SECONDS_IN_YEAR + int(YYYY)
        retval.append(temp)
...
def sounding_ids_to_decimal_year(sounding_ids):
    ...
    singleton = isinstance(sounding_ids, str) or isinstance(sounding_ids, N.int64)
...
def month_str_to_number(vals):
    ...
    for val in vals:
        ...
        if len(match) > 1:
            raise Exception('Ambiguous month string encountered %s' % val)
        if len(match) == 0:
            raise Exception('Unmatched month string encountered %s' % val)
...
def label_ranges_from_decimal_year(decimal_year, label='season', limits=None, include_year=True, year_apostrophe=False):
    ...
    if limits is not None:
        decimal_year = decimal_year[inr(decimal_year, limits[0], limits[1])]
    else:
        limits = (decimal_year.min(), decimal_year.max())

    # Need -1 to make sure we handle winter that brackets two years, plus take into consideration user-provided
    # limits (could be larger)
    ...
    # If user did specify ranges, ensure we don't return anything entirely outside them Check to see if highest range
    #  value is greater than lowest limit, and lowest range value is less than highest limit
    else:
