...
# --------------------------
def text_equation_MVLR(target_name, feature_names, slopes, intercept, slope_errors=None, intercept_error=None,
                       meaned=False, units="unitless", data_stats=None, short_names=False, format="%0.2g",
                       unicode=True):
   ...
   Returns:
    ...
    'eaten/numcandy = (-0.74)kid/height + (1.1)kid/weight + 2.1e-16'

    Short name, unitless
    >>> text_equation_MVLR ("eaten/numcandy", ['kid/height','kid/weight'], slopes, interc, short_names = True)
    'numcandy = (-0.74)height + (1.1)weight + 2.1e-16'
    ...
    u'numcandy = (-0.74 \\xb1 0.85)height + (1.1 \\xb1 0.85)weight + (2.1e-16 \\xb1 0.46)'
    ...
    'numcandy = (-0.74 +- 0.85)height + (1.1 +- 0.85)weight + (2.1e-16 +- 0.46)'
    ...
    if units in ("sensitivity", "unitless", "normalized"):
        # We assume that the input equation is already in unitless form. If the user just wished to display the
        # equation slopes directly, this is also the appropriate setting, hence default
    ...
    if not meaned:
        ...
        if slope_errors is not None:
            eq += " + ".join([("(" + format + " " + plusminus + " " + format + ")%s") % (
            slopes[i], slope_errors[i], feature_names[i]) for i in range(num)])
        ...
        if slope_errors is not None:
            eq += " + ".join([("(" + format + " " + plusminus + " " + format + ")(%s - " + format + ")") % (
            slopes[i], slope_errors[i], feature_names[i], ds['mean'][orig_feats[i]]) for i in range(num)])
            eq += " + (" + format % intercept + " " + plusminus + " " + format % intercept_error + ")"
        else:
            eq += " + ".join(
                [("(" + format + ")(%s - " + format + ")") % (slopes[i], feature_names[i], ds['mean'][orig_feats[i]])
                 for i in range(num)])
            eq += smartsign(intercept, format)
