...
def ml_dimensionality_reduction(
    ...
    if(algorithmName == "PCA"):
        try:
            result = codex_decomposition_PCA(
                inputHash, subsetHash, n_components, False, downsampled, False)
        except BaseException:
            codex_system.codex_log(
                "Failed to run PCA for dimensionality reduction analysis")
        ...
        try:
            result = codex_decomposition_PCA(
                inputHash, subsetHash, n_components, True, downsampled, False)
        except BaseException:
            codex_system.codex_log(
                "Failed to run PCA_Incremental for dimensionality reduction analysis")
        ...
        try:
            result = codex_decomposition_LDA(
                inputHash, subsetHash, n_components, downsampled, False)
        except BaseException:
            codex_system.codex_log(
                "Failed to run LDA for dimensionality reduction analysis")
        ...
        try:
            result = codex_decomposition_ICA(
                inputHash, subsetHash, n_components, downsampled, False)
        except BaseException:
            codex_system.codex_log(
                "Failed to run ICA for dimensionality reduction analysis")
        ...
        codex_system.codex_log(
            "Cannot find requested dimensionality reduction algorithm")
        return None

    return result


def codex_decomposition_PCA(
        inputHash,
        subsetHash,
        n_components,
        incremental,
    ...
    if(downsampled is not False):
        ...
        eta = codex_time_log.getComputeTimeEstimate(
            "dimension_reduction", "PCA", samples)

    data = codex_math.codex_impute(data)

    if(data.ndim > n_components):
        codex_system.codex_log(
            "ERROR: codex_dimension_reduction - PCA - features (" +
            str(
                data.ndim) +
    ...
    if(incremental):
        ...
        exp_var_ratio = codex_math.codex_explained_variance_ratio(
    ...
    if DEBUG:
        codex_plot.plot_dimensionality(
            exp_var_ratio, "PCA Explained Variance", save=True)
...
def codex_decomposition_ICA(
    ...
    if(subsetHash is not False):
        ...
        if(data is None):
            codex_system.codex_log(
                "ERROR: codex_dimmension_reudction - ICA - subsetHash returned None.")
            return None
    ...
    if(downsampled is not False):
        ...
        data = codex_downsample.downsample(data, percentage=downsampled)
        eta = codex_time_log.getComputeTimeEstimate(
            "dimension_reduction", "ICA", samples)

    data = codex_math.codex_impute(data)

    if(data.ndim > n_components):
        codex_system.codex_log(
            "ERROR: codex_dimension_reduction - ICA - features (" +
            str(
                data.ndim) +
            ") > requested components (" +
            str(n_components) +
    ...
    if(subsetHash is False):
        returnCodeString = "codex_dimmension_reduction_api.codex_decomposition_ICA('" + inputHash + "'," + str(
            False) + "," + str(n_components) + "," + str(downsampled) + ")\n"
    else:
        returnCodeString = "codex_dimmension_reduction_api.codex_decomposition_ICA('" + inputHash + "','" + str(
            subsetHash) + "'," + str(n_components) + "," + str(downsampled) + ")\n"
    codex_return_code.logReturnCode(returnCodeString)

    if showPlot:
        codex_plot.plot_dimensionality(
            exp_var_ratio, "ICA Explained Variance", show=True)

    if DEBUG:
        codex_plot.plot_dimensionality(
            exp_var_ratio, "ICA Explained Variance", save=True)
...
def codex_decomposition_LDA(
    ...
    if(subsetHash is not False):
        ...
        if(data is None):
            codex_system.codex_log(
                "ERROR: codex_dimmension_reudction - LDA - subsetHash returned None.")
            return None
    ...
    if(downsampled is not False):
        ...
        data = codex_downsample.downsample(data, percentage=downsampled)
        eta = codex_time_log.getComputeTimeEstimate(
            "dimension_reduction", "LDA", samples)

    if(data.ndim > n_components):
        codex_system.codex_log(
            "ERROR: codex_dimension_reduction - LDA - features (" +
            str(
                data.ndim) +
            ") > requested components (" +
            str(n_components) +
    ...
    if showPlot:
        codex_plot.plot_dimensionality(
            exp_var_ratio, "LDA Explained Variance", show=True)

    if DEBUG:
        codex_plot.plot_dimensionality(
            exp_var_ratio, "LDA Explained Variance", save=True)

    if(subsetHash is False):
        returnCodeString = "codex_dimmension_reduction_api.codex_decomposition_LDA('" + inputHash + "'," + str(
            False) + "," + str(n_components) + "," + str(downsampled) + ")\n"
    else:
        returnCodeString = "codex_dimmension_reduction_api.codex_decomposition_LDA('" + inputHash + "','" + str(
            subsetHash) + "'," + str(n_components) + "," + str(downsampled) + ")\n"
