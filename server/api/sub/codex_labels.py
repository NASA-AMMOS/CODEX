...
def label_swap(labels, dataHash, verbose=False):
    ...
    if(abs(ref_uniq_labels.size - test_uniq_labels.size) > 5):
        if(verbose):
            codex_system.codex_log("Difference between test and reference labels is too high, returning original labels")
        return labels
    ...
    for x in range(0,test_uniq_labels.size):
        ...
        while(finalMap[str(x)] is None):
            ...
            if(x in ref_uniq_labels):
                ...
                # Get the label currently being used in new data at ref_ind location
                cur_label = labels[ref_ind]

                # If the incoming label hasn't been used yet, use it. Else, try again.
