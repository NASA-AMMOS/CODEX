...
import codex_plot, codex_downsample
...
def ml_peak_detect(hashList, subsetHashName, algorithmName, downsampled, parms, result):
    '''
    Inputs:
    ...
    if(algorithmName == "peak_cwt"):
        ...
        try:
            result = codex_scipy_signal_peak_cwt(inputHash, subsetHash, False, False, peak_width, gap_threshold, min_snr, noise_perc)
        except:
            codex_system.codex_log("Failed to run codex_scipy_signal_peak_cwt peak detection algorithm")
    ...
    if(algorithmName == "findpeaks"):
        ...
        try:
            result = codex_matlab_findpeaks(inputHash, subsetHash, False, mph, mpd, threshold, edge, kpsh, valley, False)
        except:
            codex_system.codex_log("Failed to run matlab-findpeaks peak detection algorithm")
...
def codex_scipy_signal_peak_cwt(inputHash, subsetHash, showPlot, downsampled, peak_width, gap_threshold, min_snr, noise_perc):
    '''
    Inputs:
    ...
    if(subsetHash is not False):
        ...
        if(data is None):
            codex_system.codex_log("ERROR: codex_peak_detection scipy_signal_peak_cwt - subsetHash returned None.")
            return None
    ...
    if(subsetHash == False):
        returnCodeString = "codex_peak_detection_api.codex_scipy_signal_peak_cwt('"+inputHash+"',"+str(subsetHash)+",True,"+str(downsampled)+","+str(peak_width)+","+str(gap_threshold)+","+str(min_snr)+","+str(noise_perc)+")" 
    else:
        returnCodeString = "codex_peak_detection_api.codex_scipy_signal_peak_cwt('"+inputHash+"','"+str(subsetHash)+"',True,"+str(downsampled)+","+str(peak_width)+","+str(gap_threshold)+","+str(min_snr)+","+str(noise_perc)+")" 
	
    codex_return_code.logReturnCode(returnCodeString)

    dictionary = {'eta': eta, "indexes": indexes, "inputHash": inputHash, 'subsetHash':subsetHash, 'downsampled':downsampled}
    return dictionary

def codex_matlab_findpeaks(inputHash, subsetHash, downsampled, mph, mpd, threshold, edge, kpsh, valley, showPlot):
    '''
    Inputs:
        inputHash (string)        - Hash value corresponding to the data to cluster
        subsetHash (string)       - Hash value corresponding to the subselection (false if full feature)
    ...
    if(subsetHash is not False):
        data = codex_hash.applySubsetMask(data, subsetHash)
        if(data is None):
            codex_system.codex_log("ERROR: codex_peak_detection matlab_findpeaks - subsetHash returned None.")
            return None
    ...
    if(downsampled is not False):
        ...
        samples = len(data)
        data = codex_downsample.downsample(data, percentage=downsampled)
        eta = codex_time_log.getComputeTimeEstimate("peak", "matlab_findpeaks", samples)
    ...
    endTime = time.time()
    computeTime = endTime - startTime
    codex_time_log.logTime("peak","matlab_findpeaks",computeTime, len(data), data.ndim)

    if(showPlot == True):
        codex_plot.codex_plot_peak(data, indexes)

    if(subsetHash == False):
        returnCodeString = "codex_peak_detection_api.codex_matlab_findpeaks('"+inputHash+"',"+str(subsetHash)+","+str(downsampled)+","+str(mph)+","+str(mpd)+","+str(threshold)+",'"+str(edge)+"',"+str(kpsh)+","+str(valley)+","+str(showPlot)+")"
    else:
        returnCodeString = "codex_peak_detection_api.codex_matlab_findpeaks('"+inputHash+"','"+str(subsetHash)+"',"+str(downsampled)+","+str(mph)+","+str(mpd)+","+str(threshold)+",'"+str(edge)+"',"+str(kpsh)+","+str(valley)+","+str(showPlot)+")"
