    ...
    transformData( d, seriesI, r, reverse ) {
        if ( r ) {;
            if (reverse === "x")
                ...
                this.option.yAxis[seriesI].inverse = !(this.option.yAxis[seriesI].inverse);
            return this.option;
        }
        ...
        this.option.xAxis[seriesI].data = Array.from( new Array( Math.floor(Math.abs(xRange[1] - xRange[0]) + 1)), ( v, i ) => { return i + xRange[0] } );
        this.option.yAxis[seriesI].data = Array.from( new Array( Math.floor(Math.abs(yRange[1] - yRange[0]) + 1)), ( v, i ) => { return i + yRange[0] } );
