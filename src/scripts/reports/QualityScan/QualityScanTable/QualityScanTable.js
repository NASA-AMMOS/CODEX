...
class QualityScanTable extends Component {
  ...
  makeFeatureTable( featureList ) {
    ...
    for( let i = 0; i < featureList.length; i++ ) {
      featureTableElements.push(
        ...
        >
          <td className='tableFeatureName' title={featureList[i].name}>{featureList[i].name}</td>
   ...
   */
  getTRRef( i ) {
    if( i === 'visible' ) { //return the first visible tr
      for( let f in this.state.featureList ) {
        if( this.state.featureList[f].hidden === false )
          return this.ref_featureTableTR[f];
      }
    }
  ...
  render() {
      return (
        <div className='QualityScanTable'>
          <table id='featureTable' ref={ (r) => { this.ref_featurelist = r; } } cellPadding='0' cellSpacing='0' border='0'>
            <tbody>
              {this.makeFeatureTable(this.state.featureList)}
            </tbody>
          </table>
        </div>
      );
