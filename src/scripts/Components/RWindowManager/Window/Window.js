...
class Window extends Component {
  ...
  refresh() {
    if( this.mounted ) {
      ...
      if( this.ref_rnd && !this.justMaximized ) {
        this.ref_rnd.updateSize({ width: this.gridClose( this.width, 0 ), height: this.gridClose( this.height, 1 ) });
        this.ref_rnd.updatePosition({ x: this.gridClose( this.x, 0 ) , y: this.gridClose( this.y, 1 ) });
      }
      this.justMaximized = false;
      if( this.ref_embrace_parent ) {
        ...
        if( this.groupId !== -1 )
          ...
          this.ref_embrace_parent.style.background = '#c7c7c7';
      }
    }
  }
  ...
  tweenTo( mode, arrangement ) {
    ...
    if( mode === 'focusorder' ) {
      ...
      if( this.ref_rnd ) {
        this.ref_rnd.updateSize({ width: this.gridClose( w, 0 ), height: this.gridClose( h, 1 ) });
  ...
  updateXYWH() {
    if( this.ref_rnd && this.ref_wc ) {
      ...
      let transform = p.style.transform.replace('px','').split('(')[1].split(')')[0].split(',');
  ...
  snap(sp, edge) {
    if( !manager.snap.path.on ) {
      ...
      manager.startPath( r.x - w.x + (r.width/2), r.y - w.y + (r.height/2),
        this.props.wmId, this.props.id, edge );
    }
    else {
  ...
  minimize() {
    this.updateXYWH();
    manager.minimizeWindow( this.id,
      {
  ...
  center() {
    if( this._ref && this.ref_rnd ) {
      ...
      manager.setWindowState( this.id, null, {
        x: this.gridClose( this.x, 0 ),
        y: this.gridClose( this.y, 1 ),
        width: this.gridClose( this.width, 0 ),
        height: this.gridClose( this.height, 1 )
  ...
  maximize() {
    if( this.ref_rnd && this.ref_wc ) {
      ...
      if( !this.isMaximized ) {
        let rect = p.getBoundingClientRect();
        let transform = p.style.transform.replace('px','').split('(')[1].split(')')[0].split(',');
        this.lastWHTrans = [ rect.width, rect.height, parseInt(transform[0],10), parseInt(transform[1],10) ];
  ...
  dragEnd( e, d ) {
    manager.setWindowState( this.id, null,
      { x: this.gridClose( d.x, 0 ),
        y: Math.max( this.gridClose( d.y, 1 ), 0 ) },
      this.wmId );
  }
  resizeEnd( e, d ) {
    if( this.ref_wc ) {
      let rect = this.ref_wc.parentElement.getBoundingClientRect();
      manager.setWindowState( this.id, null, { width: rect.width, height: rect.height }, this.wmId );
    }
  }
  ...
  refreshZIndex() {
    if( this._ref )
      this._ref.style.zIndex = manager.wms[this.wmId].windows[this.id]._.zIndex;
  }
  ...
  _showHideStyle( show ) {
    return (show) ? { 'opacity': '1', 'pointerEvents': 'auto' } : { 'opacity': '0', 'pointerEvents': 'none' };
  }
  ...
  render() {
    ...
    return (
      <div className='Window' id={'Window__' + this.id}
        ref={(r) => { this._ref = r; } }>
        <Rnd
          ...
          onDragStop={(e,d) => { this.dragEnd(e,d); }}
          onResizeStop={(e,d) => { this.resizeEnd(e,d); }}
          dragHandleClassName={'.WindowHandle__' + this.id}
        >
          <div className='WindowContent' id={'WindowContent__' + this.id}
            ...
            <ContextMenuTrigger id={'WindowContextMenuHeader__' + this.id}
                holdToDisplay={-1}>
              <div className={'WindowHandle__' + this.id + ' WindowHandle'}
                ref={(wh) => { this.ref_wh = wh; } }>
                <div className='WindowHandleLeft'>
                  <div className='WindowHandleButtons'>
                    <div className='WindowHandleButtonsClose' title='close'
                      onClick={() => this.close()}
                    ></div>
                    <div className='WindowHandleButtonsMaximize' title='maximize'
                      onClick={() => this.maximize()}
                    ></div>
                    <div className='WindowHandleButtonsMinimize' title='minimize'
                      onClick={() => this.minimize()}
                    ></div>
                  </div>
                  <div className='WindowHandleTitle'>{this.title}</div>
                </div>
                <ContextMenuTrigger id={'WindowContextMenuEmbrace__' + this.id}
                  holdToDisplay={-1}>
                  <div className='WindowHandleEmbrace'
                  ref={(e) => { this.ref_embrace_parent = e; } }
                  onClick={() => this.embrace()}>
                    <div ref={(e) => { this.ref_embrace = e; } }><FaCheck /></div>
                  </div>
                </ContextMenuTrigger>
              </div>
            </ContextMenuTrigger>
            ...
            <span className='SnapCont'style={this._showHideStyle(this.props.snapped)}>
              <div className='SnapPortTop snap'
                style={this._showHideStyle(this.snaps.top)}
                ref={(sp) => { this.ref_spt = sp; } }
                onClick={() => this.snap(this.ref_spt, 'top')}
              ></div>
              <div className='SnapPortRight snap'
                style={this._showHideStyle(this.snaps.right)}
                ref={(sp) => { this.ref_spr = sp; } }
                onClick={() => this.snap(this.ref_spr, 'right')}
              ></div>
              <div className='SnapPortBottom snap'
                style={this._showHideStyle(this.snaps.bottom)}
                ref={(sp) => { this.ref_spb = sp; } }
                onClick={() => this.snap(this.ref_spb, 'bottom')}
              ></div>
              <div className='SnapPortLeft snap'
                style={this._showHideStyle(this.snaps.left)}
                ref={(sp) => { this.ref_spl = sp; } }
                onClick={() => this.snap(this.ref_spl, 'left')}
              ></div>
            </span>
          </div>
        </Rnd>
        <div className='WindowWrapper' ref={(e) => { this.wrapper = e; }}>
          <ContextMenu id={'WindowContextMenuEmbrace__' + this.id}
            className={'WindowContextMenuEmbrace'}
            onShow={() => this.correctContextMenuPosition()}
            onHide={() => this.hideContextMenu()}>
            <MenuItem data={{ action: 'embrace' }} onClick={this.handleContextMenuClick}>
              <span id={'windowcontextembrace_' + this.id}>{ this.embraced ? 'Deselect' : 'Select' }</span>
            </MenuItem>
            ...
            <MenuItem data={{ action: 'ungroupall' }} onClick={this.handleContextMenuClick}>
              Ungroup All
            </MenuItem>
            <MenuItem data={{ action: 'minimizegroup' }} onClick={this.handleContextMenuClick}>
              Minimize Group
            </MenuItem>
            <MenuItem data={{ action: 'closegroup' }} onClick={this.handleContextMenuClick}>
              Close Group
            </MenuItem>
          </ContextMenu>
          <ContextMenu id={'WindowContextMenuHeader__' + this.id}
            ...
            <MenuItem data={{ action: 'maximize' }} onClick={this.handleContextMenuClick}>
              Maximize
            </MenuItem>
