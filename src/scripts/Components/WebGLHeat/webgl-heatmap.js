...
export default class WebGL_Heatmap {
  constructor() {
    ...
    textureFloatShims = function() {
      ...
      createSourceCanvas = function() {
        ...
        else console.warn( 'imageData.data.set is undefined' );
    ...
    Heights = (function() {
      ...
      Heights.prototype.clearRect = function( x, y, width, height ) {
        this.nodeFront.use();
    ...
    this.WebGLHeatmap = (function() {
      function WebGLHeatmap(_arg) {
        var alphaEnd, alphaRange, alphaStart, error, getColorFun, gradientTexture, gradientWidth, image, intensityToAlpha, output, quad, textureGradient, _ref, _ref1;
        _ref = _arg != null ? _arg : {}, this.canvas = _ref.canvas, this.width = _ref.width, this.height = _ref.height, intensityToAlpha = _ref.intensityToAlpha, gradientTexture = _ref.gradientTexture, gradientWidth = _ref.gradientWidth, alphaRange = _ref.alphaRange;
        if (!this.canvas) {
        ...
        if (gradientTexture) {
          ...
          getColorFun =
            ...
            '\n    if( intensity == 0.0 ) { return vec4(0.0, 0.0, 0.0, 0.0); }' +
            '\n    if( intensity >= 1.0 ) { return vec4(0.0, 0.0, 0.0, 0.0); }' +
            '\n    return vec4( texture2D(gradientTexture, vec2(intensity * ' + gradientWidth + '.0, 0.0)).rgb, intensity * 255.0 );' +
            '}';
        }
        else {
          throw 'Please provide a gradientTexture';
        }
...
*/
      ...
      WebGLHeatmap.prototype.clearRect = function(x, y, width, height) {
        return this.heights.clearRect(x, y, width, height);
      };
  ...
  createWebGLHeatmap(params) {
    return new this.WebGLHeatmap(params);
  };
}

export let webglheatmap = new WebGL_Heatmap();