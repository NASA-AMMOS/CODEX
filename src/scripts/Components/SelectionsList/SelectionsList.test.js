...
describe( '<SelectionsList/>', () => {
    ...
    it('should render a list of selections', () => {
        ...
        expect(wrapper.find('li').first().text()).toEqual('Brush')
        expect(wrapper.find('li').last().text()).toEqual('sel1')
        // there should be a selected class on the first element
        expect(wrapper.find('li').first().prop('className')).toMatch(/selected/)
    ...
    it('should handle selecting a selection', () => {
        ...
        wrapper.find('li').last().find('.SelectionsList__checkbox').simulate('click')
