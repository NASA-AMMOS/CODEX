...
class Drawer extends Component {
  ...
  _draw( e ) {
    ...
    if( e.shiftKey  && e.buttons === 1 ) {
      let offset = [ coords[0] - this.vars.mouseDownPos[0], coords[1] - this.vars.mouseDownPos[1] ];
      ...
      switch( this.props.brushtype ) {
        case 'freehand':
          ...
          for( let p in this.state.polyPoints )
            newPolyPoints.push( [ this.state.polyPoints[p][0] + offset[0], this.state.polyPoints[p][1] + offset[1] ] );
          this.setState( { polyPoints: newPolyPoints } );
          break;
        case 'rectangle':
          let draggedStartPos = [ this.state.startClickPos[0] + offset[0], this.state.startClickPos[1] + offset[1] ];
          let draggedEndPos = [ this.state.endClickPos[0] + offset[0], this.state.endClickPos[1] + offset[1] ];
  ...
  _click( e, isDown ) {
    ...
    if( isDown ) {
      ...
      {
        this.setState( { isDrawing: true, startClickPos: coords, endClickPos: coords, polyPoints: [] } );
      }
    }
    else {
      this.drawingEnded(
  ...
  componentWillReceiveProps( nextProps ) {
    if( this.props.mode !== nextProps.mode )
      this.update( nextProps.mode, nextProps.brushtype )
    //Drawers know to clear their shapes if they're not the last ones drawn on
    if( this.props.windowId !== nextProps.brushId )
      this.setDrawer( 'clearshape' )
  }
  ...
  render() {
    ...
    switch( this.props.brushtype ) {
      case 'rectangle':
        let rectXYWH = [
          ...
          Math.max( this.state.startClickPos[0] - this.state.endClickPos[0],
                    this.state.endClickPos[0] - this.state.startClickPos[0] ),
          Math.max( this.state.startClickPos[1] - this.state.endClickPos[1],
                    this.state.endClickPos[1] - this.state.startClickPos[1] ),
        ];
        shape = <rect
                x={rectXYWH[0]} y={rectXYWH[1]}
                width={rectXYWH[2]} height={rectXYWH[3]}
                stroke='rgba(255,69,0,1)' strokeWidth='2' fill='rgba(0,0,0,0.1)' />
        break;
      case 'freehand':
        let polyPointsString = '';
        for( let p in this.state.polyPoints )
          polyPointsString += ' ' + this.state.polyPoints[p][0] + ',' + this.state.polyPoints[p][1];
        shape = <polygon
    ...
    return (
      <div className='Drawer' ref={(r) => { this._ref = r; } } style={drawerStyle}
        onMouseDown={ (e) => { this._click(e, true); } }
        onMouseUp={ (e) => { this._click(e, false); } }
        onMouseMove={ (e) => { this._draw(e); } }>
...
function mergeProps(propsFromState, propsFromDispatch, ownProps) {
  return {
    ...
    brushIdSet: () => { propsFromDispatch.brushIdSet(ownProps.windowId) },
