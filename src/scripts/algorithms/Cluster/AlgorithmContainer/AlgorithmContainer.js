...
class AlgorithmContainer extends Component {
  ...
  toEditParameters( on ) {
    ...
    if( this.ref_parameters )
      this.ref_parameters.toEditParameters( on );
  }
  ...
  render() {
    return (
      <div className='AlgorithmContainer'>
        <Preview
          ref={(r) => { this.ref_preview = r; }}
          algorithm={this.props.algorithm}
          invoke={ (c,r,p,v) => { this.props.invoke(c,r,p,v) } }
          updateCall={ (c,p,a) => { this.props.updateCall(c,p,a) } }
          getCall={ () => this.props.getCall() } 
          l={ (f) => { this.props.l(f) } }
          />
        <Parameters
          ref={(r) => { if( r ) this.ref_parameters = r.getWrappedInstance(); else this.ref_parameters = null; }}
          algorithm={this.props.algorithm}
          invoke={ (c,r,p,v) => { this.props.invoke(c,r,p,v) } }
          updateCall={ (c,p,a) => { this.props.updateCall(c,p,a) } }
          getCall={ () => this.props.getCall() } 
          finalInvoke={ (op) => { this.props.finalInvoke(op) } }
          setSteps={ (oa) => { this.props.setSteps(oa); } }
