...
class Cluster extends Component {
  constructor(props) {
    ...
    let dataFeatures = this.props.selectedFeatures; //getSelectedFeatures( this.props.data ).toJS();
    ...
    */
    ...
    this.chosenUpdate = this.chosenUpdate.bind(this)    
  }
  ...
  clusterInvoke( callParams, reaction, parameters, vars ) {
      ...
      if( vars )
        callCopy.identification.vars = vars;
  ...
  */
  chosenUpdate( chosenFeatures, chosenSelections ) {
    let dataFeatures = [];
    //let chosenFeatures = controller.getChosenFeatures();
    for( let i in chosenFeatures ) {
      dataFeatures.push( chosenFeatures[i].feature );
    ...
    if( dataFeatures.length <= 1 ) {
      ...
      this.ref.refreshCountdownText.innerHTML = '<div class="fillError">Please select at least 2 features to cluster</div>';
    }
    else
      this.updateCall( { dataFeatures: chosenFeatures, dataSelections: chosenSelections } );

    if( this.ref.subAlgorithms )
      this.ref.subAlgorithms.update();
    if( this.ref.algorithmContainer )
      this.ref.algorithmContainer.update();
  }
  ...
  */
  finalInvoke( outputParameters ) {
    if( !this.vars.callIsRunning ) {
      if( !outputParameters.withFeature && !outputParameters.withSelections && !outputParameters.withPCA ) {
  ...
  finalReaction( r ) {
    ...
    if( this.vars.outputPCA ) {
      this.props.featureAdd(
        this.vars.finalName + '_PCA-1',
        r.data.map( x => x[0] ),
        meta
      );
      this.props.featureAdd(
        this.vars.finalName + '_PCA-2',
        r.data.map( y => y[1] ),
        meta
      );
    }
    ...
    if( this.vars.outputGraph.with ) {
      ...
      if( this.vars.outputGraph.withClusters )
        selections = getSelectionNamesByMeta( this.props.data, 'creationId', r.cid )
          .map( s => s.get( 'name' ) );
      this.props.openGraph( 'Scatter', x, y, selections );
    }
    ...
    if( this.props.windowId !== null )
      manager.removeWindow( this.props.windowId );
  }
  ...
  setSteps( onArray ) {
    if( this.ref_steps ) {
      for( let i = 0; i < this.ref_steps.length; i++ ) {
        if( i % 2 === 0)
          this.ref_steps[i].className = 'algorithmStep';
        else
          this.ref_steps[i].className = 'stepArrow';
      }
      for( let i = 0; i < onArray.length; i++ ) {
        if( this.ref_steps[i] ) {
          if( i % 2 === 0)
            this.ref_steps[i].className = 'algorithmStep active';
          else
            this.ref_steps[i].className = 'stepArrow active';
        }
      }
    }
  }
  ...
  setSelectingSubAlgorithms( is ) {
    if( is ) {
      this.setSteps( [0] );
      if( this.ref.continueButton )
        this.ref.continueButton.innerText = 'Continue';
    }
    else
      this.setSteps( [0,1,2] );
  ...
  componentWillReceiveProps( nextProps ) {
    const chosenFeatures = nextProps.selectedFeatures;
    const chosenSelections = getActiveSelectionNames( nextProps.data ).toJS()
    if( !formulas.compareArrays(this.vars.call.dataFeatures, chosenFeatures) ||
        !formulas.compareArrays(this.vars.call.dataSelections, chosenSelections) ) {
        this.setRefreshCountdown(
          this.chosenUpdate, chosenFeatures, chosenSelections, true
        );
    }
  }
  ...
  guidanceTrigger( guidancePath ) {
    ...
    this.clusterInvoke( { routine: 'guidance', guidance: guidancePath }, this.showHelpReaction.bind(this), {} );
  }
  showHelpReaction( r ) {
  ...
  clickBack() {
    if( this.state.selectingSubAlgorithms === false &&
    ...
    if( this.state.selectingSubAlgorithms === true ) {
      if( this.ref.subAlgorithms ) {
        if( this.state.selectedSubAlgorithm !== 'None' ) {
          ...
          controller.setMessage( 'Cluster - Please choose a subalgorithm before continuing.', 'warning' );
        }
      }
    }
    else {
      if( this.ref.algorithmContainer ) {
        if( this.ref.algorithmContainer.getStage() === 'algo' ) {
          if( this.ref.continueButton )
            this.ref.continueButton.innerText = 'Run';
        }
  ...
  setRefreshCountdown( callback, chosenFeatures, chosenSelections, clearCallbacks ) {
    ...
    if( this.vars.refreshCountdownInterval )
      clearInterval( this.vars.refreshCountdownInterval );
    this.vars.refreshCountdownInterval = setInterval( () => {
      this.refreshCountdown( callback, chosenFeatures, chosenSelections );
    }, 1000 );
    this.refreshCountdown( callback, chosenFeatures, chosenSelections );
  }
  refreshCountdown( callback, chosenFeatures, chosenSelections ) {
    if( this.ref.refreshCountdown && this.ref.refreshCountdownBar && this.ref.refreshCountdownText ) {
      let fullWidth = this.ref.refreshCountdown.getBoundingClientRect().width;
      this.ref.refreshCountdownBar.style.width =
      fullWidth * ( (this.vars.refreshCountdownStart - this.vars.refreshCountdown) / this.vars.refreshCountdownStart ) + 'px';
      this.ref.refreshCountdownText.innerText = 'Features Changed - Refreshing in ' + this.vars.refreshCountdown;
    }
      ...
      this.vars.refreshCountdownTimeout = setTimeout( () => {
        ...
        if( chosenFeatures.length < 2 ) {
          ...
          for( let i = 0; i < this.vars.refreshCountdownCallbacks.length; i++ ) {
            callback( chosenFeatures, chosenSelections )
          }
  ...
  setContinueOpacity( opacity ) {
    if( this.ref.continue )
      this.ref.continue.style.opacity = opacity;
  }
      ...
      if( this.ref.runLoadingBar ) {
        ...
        this.vars.runTimeInterval = setInterval( () => {
          ...
          if( !this.vars.isExtraTime )
            this.ref.runLoadingBar.style.width = ( ( this.vars.currentRunTime / fullWidth ) * 100 ) + '%';
          else
            this.ref.runLoadingBarExtra.style.width = ( ( fullWidth * ( ( this.vars.currentRunTime - finishedRunTime ) / this.vars.currentRunTime ) ) / fullWidth * 100 ) + '%';

          let textAction = 'Clustering';
          for( let i = 1; i < ( ( counts / 50 ) % 4 ); i++ ) {
            textAction += '.';
          }
          this.ref.runLoadingTextAction.innerHTML = textAction;
          if( !this.vars.isExtraTime )
            this.ref.runLoadingTextTime.innerHTML = parseInt( estimatedTime - (counts/rate), 10 ) + 's Remaining';
          else
            this.ref.runLoadingTextTime.innerHTML = parseInt( (counts/rate) - estimatedTime, 10 ) + 's Overtime';
          ...
          this.ref.runLoadingTextTime.style.textShadow =
		        '-1px -1px 0 rgb('+r+','+r+','+r+'), 1px -1px 0 rgb('+r+','+r+','+r+'), -1px 1px 0 rgb('+r+','+r+','+r+'), 1px 1px 0 rgb('+r+','+r+','+r+')';
  ...
  setToOutputs() {
    if( this.state.selectedSubAlgorithm !== 'None' ) {
      ...
      if( this.ref.algorithmContainer )
        this.ref.algorithmContainer.toEditParameters( false );
  ...
  disableInteractions() {
    ...
    if( this.ref.algorithmContainer )
      this.ref.algorithmContainer.disable();
  ...
  enableInteractions() {
    ...
    if( this.ref.algorithmContainer )
      this.ref.algorithmContainer.enable();
  ...
  render() {
    ...
    if( !this.state.selectingSubAlgorithms ) {
      algorithmContainer = <AlgorithmContainer
        ref={(r) => { this.ref.algorithmContainer = r; }}
        algorithm={this.state.selectedSubAlgorithm}
        invoke={ (c,r,p,v) => { this.clusterInvoke(c,r,p,v) } }
        updateCall={ (c,p,a) => { this.updateCall(c,p,a) } }
        getCall={ () => this.getCall() } 
        l={ (f) => { this.subscribeOverallPreviewUpdate(f) } }
        setRun={ (on) => { this.setRun(on); } }
        finalInvoke={ (op) => { this.finalInvoke(op); } }
        setSteps={ (oa) => { this.setSteps(oa); } }
      ...
      <div className={clusterClass}
        ...
        <div className='right'>
          <div id='title'>
            ...
            <div id='algorithmSteps' ref={ (r) => { this.ref_algosteps = r; } }>
              <div className='algorithmStep active' ref={ (r) => { this.ref_steps[0] = r; } }
                onClick={ () => { if( this.ref.subAlgorithms ) this.ref.subAlgorithms.toggleSubAlgorithmSelector(true); } }
              >Choose Algorithm</div>
              <div className='stepArrow' ref={ (r) => { this.ref_steps[1] = r; } }><MdArrowForward /></div>
              <div className='algorithmStep' ref={ (r) => { this.ref_steps[2] = r; } }
                onClick={ () => { this.setToEditParameters(); } }
              >Edit Parameters</div>
              <div className='stepArrow' ref={ (r) => { this.ref_steps[3] = r; } }><MdArrowForward /></div>
              <div className='algorithmStep' ref={ (r) => { this.ref_steps[4] = r; } }
                onClick={ () => { this.setToOutputs(); } }
                >Outputs</div>
              <div className='stepArrow' ref={ (r) => { this.ref_steps[5] = r; } }><MdArrowForward /></div>
              <div className='algorithmStep' ref={ (r) => { this.ref_steps[6] = r; } }>Run</div>
            </div>
          </div>
          <div className='bottomBar'>
            <div className='loadingNotRunning'
              ref={(r) => { this.ref.loadingNotRunning = r; }}>
              <div className='bottomBarBack'
                onClick={() => this.clickBack()}
                style={{opacity: backbuttonOpacity}}
              ><MdArrowBack /> <span>Back</span></div>
              <div className='refreshCountdown' ref={(r) => { this.ref.refreshCountdown = r; }}>
                  <div className='refreshCountdownBar'ref={(r) => { this.ref.refreshCountdownBar = r; }}></div>
                  <div className='refreshCountdownText' ref={(r) => { this.ref.refreshCountdownText = r; }}></div>
              </div>
              <div className='bottomBarContinue'
                ref={(r) => { this.ref.continue = r; }}
                onClick={() => this.clickContinue()}
              ><span ref={(r) => { this.ref.continueButton = r; }}>Continue</span> <MdArrowForward /></div>
            </div>
            <div className='loadingRunning'
              ref={(r) => { this.ref.loadingRunning = r; }}>
              <div className='runLoading' ref={(r) => { this.ref.runLoading = r; }}>
                  <div className='runLoadingBars'>
                    <div className='runLoadingBar' ref={(r) => { this.ref.runLoadingBar = r; }}></div>
                    <div className='runLoadingBarExtra' ref={(r) => { this.ref.runLoadingBarExtra = r; }}></div>
                  </div>
                  <div className='runLoadingTextAction' ref={(r) => { this.ref.runLoadingTextAction = r; }}></div>
                  <div className='runLoadingTextTime' ref={(r) => { this.ref.runLoadingTextTime = r; }}></div>
              </div>
            </div>
          </div>
          <SubAlgorithms
              ref={(r) => { this.ref.subAlgorithms = (r) ? r.getWrappedInstance() : null; }}
              invoke={ (c,r,p,v) => { this.clusterInvoke(c,r,p,v) } }
              updateCall={ (c,p,a) => { this.updateCall(c,p,a) } }
              setSelectingSubAlgorithms={ (is) => { this.setSelectingSubAlgorithms(is) } }
              setSelectedSubAlgorithm={ (s) => { this.setSelectedSubAlgorithm(s) } }
              setSteps={ (oa) => { this.setSteps(oa); } }
              setContinueOpacity={ (o) => { this.setContinueOpacity(o); } }
              setEstimatedTime={ (t) => { this.setEstimatedTime(t); } }
            />
          {algorithmContainer}
        </div>
        <Help
          ref={(r) => { this.ref_help = r; }}
          parent={() => document.getElementsByClassName('Cluster')[0].parentElement}
...
const mapDispatchToProps = (dispatch) => ({
  ...
  selectionCreate: (name, mask, visible, color, meta) => dispatch(selectionCreate(name, mask, visible, color, meta))
})
function mergeProps(propsFromState, propsFromDispatch, ownProps) {
  return {
    data: propsFromState.data,
    ui: propsFromState.ui,
    selectedFeatures: propsFromState.selectedFeatures,
    ...
    featureAdd: propsFromDispatch.featureAdd,
    selectionCreate: propsFromDispatch.selectionCreate,
