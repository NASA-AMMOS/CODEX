...
class Filter extends Component {
    ...
    createExprTree(input) {
        ...
        try {
			...
			if (invalid.length !== 0) {
				throw new Error(`Unknown feature${invalid.length > 1 ? 's' : ''}: ${invalid.join(', ')}`)
			}
    ...
    createExprString(node) {
        ...
        if (node.type === 'Compound') {
            ...
            return this.createExprString(node.callee) + '(' + node.arguments.map(this.createExprString).join(', ') + ')'
        } else if (node.type === 'UnaryExpression') {
            return node.operator + this.createExprString(node.argument)
        } else if (node.type === 'LogicalExpression') {
            return `(${this.createExprString(node.left)} ${node.operator} ${this.createExprString(node.right)})`
        } else if (node.type === 'BinaryExpression') {
            return `(${this.createExprString(node.left)} ${node.operator} ${this.createExprString(node.right)})`
        } else if (node.type === 'ConditionalExpression') {
            return '(' + this.createExprString(node.test) + ' ? ' + this.createExprString(node.consequent) + ' : ' + this.createExprString(node.alternate) + ')'
	...
	evaluateExpression(expression) {
		...
		for (let ident of identifiers) {
			cols[ident] = getColumn(this.props.data, ident).toJS()
    ...
    render() {
        ...
        const create_heat_row = (expr, coverage) => (
            <tr key={expr}>
                <td>{expr}</td>
				<td>{
					this.state.expr_masks[expr] === null || this.state.expr_masks[expr] === undefined ? 
        ...
        return (
            <div className="Filter">
                <div className={classNames({
                    ...
                    <span className="Filter__expr-features">available features: {available_features.join(', ')}</span>
...
const mapStateToProps = (state) => {
	...
	return {
		data: domain,
		selectedFeatures: getSelectedFeatures(domain)
	}
}
