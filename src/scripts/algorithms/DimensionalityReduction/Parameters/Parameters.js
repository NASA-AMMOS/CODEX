...
class Parameters extends Component {
  constructor(props) {
    ...
    var p = controller.getAlgoSubalgo( 'Dimensionality Reduction', this.props.algorithm );

    if( p && p.hasOwnProperty( 'parameters') ) p = p.parameters
    else p = [];
    ...
    for( let p in this.t.parameters ) {
      ...
      if( this.t.parameters[p].name === 'n_components' )
        defaultParameters[this.t.parameters[p].name] = getSelectedFeatures( this.props.data ).size;
    }
    ...
    controller.subscribeChosenFeaturesUpdate( this.chosenUpdate.bind(this), 'parameters' );
    controller.subscribeChosenSelectionsUpdate( this.chosenUpdate.bind(this), 'parameters' );
  }
  ...
  reaction( r ) {
    if( this.state.openParameterI !== -1 ) {
      if( this.samples[r.identification.vars.i][r.identification.vars.j] ) {
        ...
        this.echarts_react[r.identification.vars.i][r.identification.vars.j].getEchartsInstance().clear();
        ...
        this.echarts_react[r.identification.vars.i][r.identification.vars.j].getEchartsInstance().hideLoading();
        try {
          this.echarts_react[r.identification.vars.i][r.identification.vars.j].getEchartsInstance().setOption( g.getOption() );
        } catch(e){}
      }
    }
  }
  ...
  chosenUpdate() {
    if( this.state.setOpenParameterI !== -1 )
      this.setOpenParameterI( 'refresh' );
  }
  ...
  setParameterItoValue( i, value ) {
      ...
      let setMoresClasses = ( i, activeValue ) => {
        for( let m in this.ref_mores[i] ) {
          if( m == activeValue.toString() ) //eslint-disable-line eqeqeq
            this.ref_mores[i][m].className = 'mores active';
          else
            this.ref_mores[i][m].className = 'mores';
        }
      }

      setMoresClasses( i, value );
      
      if( this.ref_inputs[i] )
        this.ref_inputs[i].value = parseFloat(value);
  ...
  _makeMoresForParameter( index ) {
    ...
    p.max = getSelectedFeatures( this.props.data ).size;

    for( let i = p.max; i >= p.min; i -= p.step ) {
      ...
      mores.push(
        <div key={i} className={classes}
          ref={(r) => { this.ref_mores[index][i] = r; }}
          onClick={ ()=> { this.setParameterItoValue( index, i ) } }>
          <div id='name'>{i}</div>
          <div id='sample' ref={(e) => { this.samples[index][i] = e; } }>
            <ReactEcharts ref={(e) => { this.echarts_react[index][i] = e; }}
              showLoading={true}
        ...
        <div key={i} className='parameter'>
          <div>
            <li>
              ...
              <div id='paramtoolbar'>
                <input type={this.t.parameters[i].inputType}
                  ref={(r) => { this.ref_inputs[i] = r; }}
                  defaultValue={
                    ( this.t.parameters[i].name === 'n_components' ) ? getSelectedFeatures( this.props.data ).size : this.state.parameterValues[i]
                  } 
                  min={this.t.parameters[i].min}
                  max={this.t.parameters[i].max}
                  step={this.t.parameters[i].step}
                  onChange={ (e) => { this.setParameterItoValue( i, e.target.value ) } } />
                <div id='more'
                  onClick={ () => { this.setOpenParameterI( i ) } }
                ><MdMoreVert /></div>
              </div>
            </li>
            <li className='parammore' ref={ (r) => { this.ref_parameterlists[i] = r; } }>
              <Collapse isOpened={this.state.openParameterI === i}>
                <div className='parammoreli'
                  onWheel={(e) => this.wheel(e, i)}>
  ...
  setOpenParameterI( i ) {
    ...
    if( i !== -1 ) {
      ...
      if( p ) {
        for( let j = p.max; j >= p.min; j -= p.step ) {
          ...
          this.props.invoke( {}, this.reaction.bind(this), invokeParameters, { i: i, j: j } );
        }
      }
    }
    else shouldSetState = true;
    if( shouldSetState )
      this.setState( { openParameterI: i } );
  }
  ...
  back() {
    if( this.vars.parameterType === 'output' )
      this.toggleParameterType( 'algo' );
  }
  continue() {
    if( this.vars.parameterType === 'algo' ) {
  ...
  makeFeaturesDropdown( axis ) {
    ...
    function makeNewMenuItems() {
      let items = [];
      items.push( <MenuItem header key={'a'}>New</MenuItem> );
      ...
      items.push(
        <MenuItem key={'aa'}
          ...
          onSelect={ () => {
              ...
              document.getElementsByClassName( 'parameterDropdownToggle' + axis )[0].innerText = op.name + '_PCA1';
            } }>
          {op.name + '_PCA1'}
        </MenuItem>
      );
      items.push(
        <MenuItem key={'ab'}
          ...
          onSelect={ () => {
              ...
              document.getElementsByClassName( 'parameterDropdownToggle' + axis )[0].innerText = op.name + '_PCA2';
            } }>
          {op.name + '_PCA2'}
        </MenuItem>
      );
      items.push(
        <MenuItem key={'ac'}
          ...
          onSelect={ () => {
              ...
              document.getElementsByClassName( 'parameterDropdownToggle' + axis )[0].innerText = op.name + '_ClusterIDs';
            } }>
          {op.name + '_ClusterIDs'}
        </MenuItem>
      );
    ...
    function makeExistingMenuItems() {
      let items = [];
      items.push( <MenuItem header key={'b'}>Existing</MenuItem> );
        ...
        items.push(
          <MenuItem key={i}
            onSelect={ () => {
                ...
                document.getElementsByClassName( 'parameterDropdownToggle' + axis )[0].innerText = featureNames[i];
              } }>
            {featureNames[i]}
          </MenuItem>
        );
    ...
    return (
      <Dropdown
            className='parametersDropdownFeatures'
            autoOpen={false}
          >
          <Dropdown.Toggle className={'parameterDropdownToggle' + axis}
            ref={ (r) => { this.ref_dropdown[axis] = r; } } title={ axis + ' Feature' } />
  ...
  nameChange( name ) {
    ...
    document.getElementsByClassName( 'parameterDropdownItem_' + 'X' + '_PCA1' )[0].children[0].innerText = name + '_PCA1';
    document.getElementsByClassName( 'parameterDropdownItem_' + 'X' + '_PCA2' )[0].children[0].innerText = name + '_PCA2';
    document.getElementsByClassName( 'parameterDropdownItem_' + 'X' + '' )[0].children[0].innerText = name + '_ClusterIDs';
    document.getElementsByClassName( 'parameterDropdownItem_' + 'Y' + '_PCA1' )[0].children[0].innerText = name + '_PCA1';
    document.getElementsByClassName( 'parameterDropdownItem_' + 'Y' + '_PCA2' )[0].children[0].innerText = name + '_PCA2';
    document.getElementsByClassName( 'parameterDropdownItem_' + 'Y' + '' )[0].children[0].innerText = name + '_ClusterIDs';
  }
  pcaChange() {
  ...
  componentWillUnmount() {
    controller.unsubscribeChosenFeaturesUpdate( this.chosenUpdate.bind(this), 'parameters' );
    controller.unsubscribeChosenSelectionsUpdate( this.chosenUpdate.bind(this), 'parameters' );
  }
  ...
  render() {
    ...
    return (
      <div className='Parameters' ref={ (r) => { this.ref_parameters = r; } }>
        <div id='algorithmParameters' ref={ (r) => { this.ref_algoParam = r; } }>
          <div id='title'>Algorithm Parameters</div>
          <div id='container'>
            {this._makeParameters()}
          </div>
        </div>
        <div id='outputParameters' ref={ (r) => { this.ref_outputParam = r; } }>
          ...
          <div className='outputParameter'>
            ...
            <input type='text'
                ...
                onInput={ (e) => { this.nameChange(e.target.value) } }
              />
          </div>
