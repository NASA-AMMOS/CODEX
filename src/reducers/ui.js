...
import { manager } from '../scripts/Components/RWindowManager/manager/manager';

let config;
if(process.env.NODE_ENV === 'test') config = require('../scripts/config.mock').config;
else config = require('../scripts/config').config;
...
const openGraph = (state, action) => {
	...
	*/
	...
	const selectedFeatures = dataSels.getSelectedFeatures(action.dataState).toJS();
	...
	const activeSelectionNames = dataSels.getActiveSelectionNames(action.dataState).map(sel => sel.get('name')).toJS()
...
const openDevelopment = ( state, action ) => {
	switch( action.name.toLowerCase() ) {
		case 'nrandomscatters':
				...
				for( let i = 0; i < r; i++ ) {
					openGraph( state, { dataState: action.dataState, name: 'Scatter', randomFeatures: true } );
				}
			break;
		case 'sparklinerange':
			manager.addWindow({
